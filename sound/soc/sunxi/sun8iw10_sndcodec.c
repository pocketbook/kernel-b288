...
static int sunxi_machine_probe(struct platform_device *pdev)
{
	...
	int ret = 0;
	struct device_node *np = pdev->dev.of_node, *ap;
	if (!np) {
		dev_err(&pdev->dev,
	...
	ap = of_find_node_by_name(NULL,"AUDIO_GPIO_PARA");
	if (ap) {
		jack_gpios[0].gpio = of_get_named_gpio_flags(ap, "headphone_detect_gpio", 0, (enum of_gpio_flags *)&config);
	...
	if (!gpio_is_valid(jack_gpios[0].gpio)) {
		...
		pr_debug("jack_gpio num is %d \n gpio = %i; mul = %i; pul = %i; drv = %i; data = %i", jack_gpios[0].gpio, config.gpio,config.mul_sel,
