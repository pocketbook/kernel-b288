...
struct sunxi_daudio_info {
	...
	struct pinctrl_state *ac0_pin_en;
	struct pinctrl_state *ac0_pin_sleep;
	struct pinctrl_state *ac1_pin_en;
	struct pinctrl_state *ac1_pin_sleep;
	struct sunxi_daudio_platform_data *pdata;
	unsigned int hub_mode;
	unsigned int hdmi_en;
	unsigned int extclk_en;
};
			...
			int clk_id, unsigned int freq, int dir)
{
	...
	if (!sunxi_daudio->extclk_en) {
		...
		if(freq == 24576000)
			sunxi_daudio->pllclk = extclk0;
		else
			sunxi_daudio->pllclk = extclk1;
		if (clk_set_parent(sunxi_daudio->moduleclk, sunxi_daudio->pllclk)) {
			pr_err("set parent of moduleclk to pllclk failed\n");
		}
		clk_prepare_enable(sunxi_daudio->pllclk);
		clk_prepare_enable(sunxi_daudio->moduleclk);

		if(freq == 24576000) {
			ret = pinctrl_select_state(sunxi_daudio->pinctrl, sunxi_daudio->ac1_pin_sleep);
			if (ret)
				pr_warn("digital audio set ac1_pin_sleep  state failed\n");

			ret = pinctrl_select_state(sunxi_daudio->pinctrl, sunxi_daudio->ac0_pin_en);
			if (ret)
				pr_warn("digital audio set ac0_pin_en  state failed\n");
		} else {
			ret = pinctrl_select_state(sunxi_daudio->pinctrl, sunxi_daudio->ac0_pin_sleep);
			if (ret)
				pr_warn("digital audio set ac0_pin_sleep  state failed\n");
 
			ret = pinctrl_select_state(sunxi_daudio->pinctrl, sunxi_daudio->ac1_pin_en);
			if (ret)
				pr_warn("digital audio set ac0_pin_en  state failed\n");
		}
	}
...
static int sunxi_daudio_dev_probe(struct platform_device *pdev)
{
	...
	ret = of_property_read_u32(np, "extclk_en", &temp_val);
	if (ret < 0) {
		dev_warn(&pdev->dev, "extclk_en  missing or invalid\n");
		sunxi_daudio->extclk_en = 0;
	} else {
		sunxi_daudio->extclk_en = temp_val;
	}
	...
	if (sunxi_daudio->extclk_en) {
		extclk0 = of_clk_get(np, 2);
		if (IS_ERR_OR_NULL(extclk0)) {
			dev_err(&pdev->dev, "extclk0 get failed\n");
			ret = PTR_ERR(extclk0);
			goto err_iounmap;
		}
		extclk1 = of_clk_get(np, 3);
		if (IS_ERR_OR_NULL(extclk1)) {
			dev_err(&pdev->dev, "extclk1 get failed\n");
			ret = PTR_ERR(extclk1);
			goto err_iounmap;
		}
	}
	...
	if (sunxi_daudio->pdata->external_type) {
		...
		if (sunxi_daudio->extclk_en) {
			sunxi_daudio->ac0_pin_en = pinctrl_lookup_state(sunxi_daudio->pinctrl, "ac0_pin_en");
			if (IS_ERR_OR_NULL(sunxi_daudio->ac0_pin_en)) {
				dev_err(&pdev->dev, "pinctrl extclk0 state get failed\n");
				ret = -EINVAL;
				goto err_pinctrl_put;
			}

			sunxi_daudio->ac0_pin_sleep = pinctrl_lookup_state(sunxi_daudio->pinctrl, "ac0_pin_sleep");
			if (IS_ERR_OR_NULL(sunxi_daudio->ac0_pin_sleep)) {
				dev_err(&pdev->dev, "pinctrl extclk0 sleep state get failed\n");
				ret = -EINVAL;
				goto err_pinctrl_put;
			}

			sunxi_daudio->ac1_pin_en = pinctrl_lookup_state(sunxi_daudio->pinctrl, "ac1_pin_en");
			if (IS_ERR_OR_NULL(sunxi_daudio->ac1_pin_en)) {
				dev_err(&pdev->dev, "pinctrl extclk1 state get failed\n");
				ret = -EINVAL;
				goto err_pinctrl_put;
			}

			sunxi_daudio->ac1_pin_sleep = pinctrl_lookup_state(sunxi_daudio->pinctrl, "ac1_pin_sleep");
			if (IS_ERR_OR_NULL(sunxi_daudio->ac1_pin_sleep)) {
				dev_err(&pdev->dev, "pinctrl extclk1 sleep state get failed\n");
				ret = -EINVAL;
				goto err_pinctrl_put;
			}
		}
