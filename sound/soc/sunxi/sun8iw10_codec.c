...
//hxm add
int Audio_gpio_sysconfig_para(void)
{
	struct device_node *np = NULL;
	int ret = -1;
	//enum of_gpio_flags fl;
	struct gpio_config fl;

	np = of_find_node_by_name(NULL,"AUDIO_GPIO_PARA");
	if (!np) {
		pr_err("ERROR! get SPEAKER_PARA failed, func:%s, line:%d\n",__FUNCTION__, __LINE__);
		goto devicetree_get_item_err;
	}

	hp_gpio.gpio = of_get_named_gpio_flags(np, "headphone_en_gpio", 0, &fl);
	if (!gpio_is_valid(hp_gpio.gpio)){
		pr_err("%s: hp_gpio is invalid. \n",__func__ );
		goto devicetree_get_item_err;
	}
	printk("hp_gpio.gpio =%d \n",hp_gpio.gpio);
	of_node_put(np);

	if (hp_gpio.gpio != -1) {
		ret = gpio_request(hp_gpio.gpio, "headphone_gpio");
		if (ret) {
			printk("Failed to request hp_gpio :%d, ERRNO:%d", (int)hp_gpio.gpio, ret);
			ret = -ENODEV;
			goto devicetree_get_item_err;
		}
		ret = gpio_direction_output(hp_gpio.gpio,0);	
		if (ret) {
			ret = -ENODEV;
			goto devicetree_get_item_err;
		}
...
void audio_fun_start_stop(struct snd_soc_codec *codec,int mute){
	printk("[%s] mute = %i; hp_gpio.gpio = %u", __func__, mute, hp_gpio.gpio);

	if(mute){
		snd_soc_update_bits(codec, HP_VOL, (0x3F<<HPVOL),0);
		msleep(50);
		if (hp_gpio.cfg) {
			//gpio_set_value(hp_gpio.gpio, 0);
			gpio_direction_input(hp_gpio.gpio);
		}
		if (spk_gpio.cfg)
			gpio_direction_input(spk_gpio.gpio);
...
//		snd_soc_write(codec, SPKR_CTR, 0x1F);
	}else{
//		msleep(50);
...
//		msleep(50);
		...
		if (hp_gpio.cfg)
			gpio_direction_output(hp_gpio.gpio, 1);
			//gpio_set_value(hp_gpio.gpio, 1);

		msleep(50);
		snd_soc_update_bits(codec, HP_VOL, (0x3F<<HPVOL),
				0x3f << HPVOL);
		msleep(50);
//		snd_soc_write(codec, SPKR_CTR, 0xFF);
	}
...
//		snd_soc_update_bits(codec, HP_CTRL, (0x3<<COS_SLOPE_CTRL), (0x3<<COS_SLOPE_CTRL));
	} else {
	...
	*/
	snd_soc_update_bits(codec, SUNXI_DAC_FIFOC,
			(0x3<<DAC_DRQ_CLR_CNT), (0x3<<DAC_DRQ_CLR_CNT));
	snd_soc_update_bits(codec, SUNXI_DAC_FIFOC,
			(0x1<<FIFO_FLUSH), (0x1<<FIFO_FLUSH));
	/*
	*	0:64-Tap FIR
	*	1:32-Tap FIR
	*/
	snd_soc_update_bits(codec, SUNXI_DAC_FIFOC,
			(0x1<<FIR_VER), (0x0<<FIR_VER));

	snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
			(0x1<<ADC_FIFO_FLUSH), (0x1<<ADC_FIFO_FLUSH));
	snd_soc_update_bits(codec, PA_POP_CTR,
			(0x7<<PA_POP_CTRL), (0x04<<PA_POP_CTRL)); //655 ms antipop when power enable
...
//			(sunxi_internal_codec->gain_config.headphonevol<<HPVOL));
    ...
    //hxm add
	snd_soc_update_bits(codec, MIC_GCTR, (0x7<<4), (sunxi_internal_codec->gain_config.maingain<<4));
	snd_soc_update_bits(codec, BIAS_MIC_CTR, (0x7<<0), (sunxi_internal_codec->gain_config.maingain<<0));
	snd_soc_update_bits(codec, LOMIX_SRC, (0x1<<6), (1<<6));
	snd_soc_update_bits(codec, ROMIX_SRC, (0x1<<6), (1<<6));
	//end
	return 0;
}
			  ...
			  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		if (sunxi_internal_codec->adc_enable == 0) {
			/* should flush adc fifo before enable ADC */
			snd_soc_update_bits(codec, SUNXI_DAC_FIFOC,
					(0x1<<FIFO_FLUSH), (0x1<<FIFO_FLUSH));
			snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
					(0x1<<EN_AD), (0x1<<EN_AD));
	...
	case SND_SOC_DAPM_POST_PMD:
		if (sunxi_internal_codec->adc_enable > 0) {
			...
			if (sunxi_internal_codec->adc_enable == 0) {
				snd_soc_update_bits(codec, SUNXI_ADC_FIFOC,
					(0x1<<EN_AD), (0x0<<EN_AD));
				/* should flush adc fifo after disable ADC */
				snd_soc_update_bits(codec, SUNXI_DAC_FIFOC,
					(0x1<<FIFO_FLUSH), (0x1<<FIFO_FLUSH));
			}
		}
				...
				int event)
{
	...
	printk("hxm add speaker power state change.event=%d\n",event);
	switch (event) {
		case SND_SOC_DAPM_POST_PMU:
...
static const struct snd_kcontrol_new sunxi_codec_controls[] = {
	SOC_SINGLE_TLV("digital volume", SUNXI_DAC_DPC, DVOL, 0x3f, 0, dig_vol_tlv),
...
static const char *ac_rspks_func_sel[] = {
	"NULL", "MIXER_L Switch", "MIXER_R Switch", "MIXR+MIXL"};
static const struct soc_enum ac_rspks_func_enum =
	SOC_ENUM_SINGLE(SPKR_CTR, SPKR_SS, 4, ac_rspks_func_sel);
...
static const struct soc_enum ac_lspks_func_enum =
	SOC_ENUM_SINGLE(SPKL_CTR, SPKL_SS, 4, ac_lspks_l_func_sel);
...
static const struct snd_soc_dapm_widget ac_dapm_widgets[] = {
	...
	/*headphone*/
	SND_SOC_DAPM_HP("HP", ac_headphone_event),
	/*speaker*/
...
static int codec_mute(struct snd_soc_dai *codec_dai, int mute)
{
	struct snd_soc_codec *codec = codec_dai->codec;
	struct sunxi_codec *sunxi_internal_codec = snd_soc_codec_get_drvdata(codec);
	printk("hxm add codec_mute=%d,sunxi_internal_codec->pa_sleep_time=%d\n",mute,sunxi_internal_codec->pa_sleep_time);
	audio_fun_start_stop(codec,mute);
...
//	}
	...
	if (!IS_ERR(sunxi_internal_codec->vol_supply.pa_shdn)) {

		regulator_disable(sunxi_internal_codec->vol_supply.pa_shdn);
	}
...
//	//turn off HP amplifier to reset it
//	if (sunxi_internal_codec->moduleclk != NULL) {
...
//	}
	...
	if (!IS_ERR(sunxi_internal_codec->vol_supply.pa_shdn)) {
		ret = regulator_enable(sunxi_internal_codec->vol_supply.pa_shdn);
		if (ret) {
...
static struct sunxi_codec *sunxi_codec_internal = NULL;
void reset_audio_codec() {
	if (sunxi_codec_internal == NULL)
		return;

	struct sunxi_codec *sunxi_internal_codec = sunxi_codec_internal;
...
EXPORT_SYMBOL(reset_audio_codec);
...
static int  sunxi_internal_codec_probe(struct platform_device *pdev)
{
	...
	/*voltage*/
	sunxi_internal_codec->vol_supply.pa_shdn =  regulator_get(NULL, "pa-shdn");/*HPVCC*/
	if (IS_ERR(sunxi_internal_codec->vol_supply.pa_shdn)) {
		pr_err("get audio pa-shdn failed\n");
	} else {
		ret = regulator_set_voltage(sunxi_internal_codec->vol_supply.pa_shdn , 3300000 , 3300000);
		if (ret) {
	...
	//save for external reset
	sunxi_codec_internal = sunxi_internal_codec;
	return 0;
err1:
