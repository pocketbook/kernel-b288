...
void *fat_follow_link(struct dentry *dentry, struct nameidata *nd) {
	...
	res = fat_getlink(dentry, NULL, 0, &buffer);
//	printk("[%s] fat_getlink name=%s return = %i; buf = %p \"%s\"; count=%i countND=%i\n", __func__,
//			dentry->d_iname,res,buffer,buffer,dentry->d_count, nd->path.dentry->d_count);
	if (! buffer) return ERR_PTR(-ENOENT);

	res = vfs_follow_link(nd, buffer);
	if (res == -ENOENT) {
		path_get(&nd->path);
		printk("[%s] vfs_follow_link nameND=%s return = %i; countND=%i\n", __func__, nd->path.dentry->d_iname,res,nd->path.dentry->d_count);
		printk("[%s] symlink error! update counter to avoid exception\n", __func__);
	}
