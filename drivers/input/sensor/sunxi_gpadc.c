...
int Get_ADC_mVoltage(void) {
	int timeout = 100;
	bool data_ready = 0;
	int data = -ETIMEDOUT;

	struct sunxi_gpadc *sunxi_gpadc_handler = sunxi_gpadc_get_handler();
	...
	sunxi_gpadc_mode_select(sunxi_gpadc_handler->reg_base, GP_SINGLE_MODE);
	sunxi_gpadc_ch_select(sunxi_gpadc_handler, GP_CH_0);

	//clear adc int status
	sunxi_gpadc_clr_ints(sunxi_gpadc_handler->reg_base, GP_CH0_DATA_IRQ_PEND);

	//start adc work
	sunxi_gpadc_en(sunxi_gpadc_handler, true);

	while(--timeout) {
		data_ready = (GP_CH0_DATA_IRQ_PEND & sunxi_gpadc_read_ints(sunxi_gpadc_handler->reg_base));
		if (data_ready)
			break;
	...
	if (data_ready)
		data = sunxi_gpadc_read_data(sunxi_gpadc_handler->reg_base, GP_CH_0);

	sunxi_gpadc_clr_ints(sunxi_gpadc_handler->reg_base, GP_CH0_DATA_IRQ_PEND);
	sunxi_gpadc_en(sunxi_gpadc_handler, false);
	//ref voltage is 3V, conversion k = 3 * 1000 / (2^12)
	return (data_ready && data >= 0) ? (data * 732 / 1000) : data;
}
EXPORT_SYMBOL_GPL(Get_ADC_mVoltage);
...

{
	...
	sunxi_gpadc_en(p_sunxi_gpadc, false);
...
static int sunxi_gpadc_resume(struct device *dev)
{
	...
	sunxi_gpadc_en(p_sunxi_gpadc, true);
