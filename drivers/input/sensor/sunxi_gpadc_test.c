...
static int __init gp_test_init(void)
{
	...
	val = sunxi_gpadc_read_ints(sunxi_gpadc_handler->reg_base);
	v = sunxi_gpadc_read_data(sunxi_gpadc_handler->reg_base, GP_CH_0);
	printk("1 - int = %x, v = %x\n",val,v);
...
//	printk("2 - int = %x, v = %x\n",val,v);
	...
	while (try_time--) {
			...
			mdelay(10);
			if (val != 0) {
				sunxi_gpadc_clr_ints(sunxi_gpadc_handler->reg_base,1);
