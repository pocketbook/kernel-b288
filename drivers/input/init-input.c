...
static int ctp_fetch_sysconfig_para(enum input_sensor_type *ctp_type)
{
	...
	pr_err(
		"CTP: %s @twi%d (%dx%d) irq=%d power_ldo=%s:%d power_io=<GP%d.M%d.P%d.S%d.V%d>\n",
		data->name,
		data->twi_id,
		data->screen_max_x,
		data->screen_max_y,
		...
		data->ctp_power,
		data->ctp_power_vol,
		data->ctp_power_io.gpio,
		data->ctp_power_io.mul_sel,
		data->ctp_power_io.pull,
		data->ctp_power_io.drv_level,
		data->ctp_power_io.data
	);
	return 0;
...
static int ctp_init_platform_resource(enum input_sensor_type *ctp_type)
{
	...
	if (data->ctp_power) {
		...
		if (!data->ctp_power_ldo)
			...
			ret = regulator_set_voltage(data->ctp_power_ldo,
					(int)(data->ctp_power_vol)*1000,
					(int)(data->ctp_power_vol)*1000);
			//ret = regulator_enable(data->ctp_power_ldo);
			msleep(1);
	} else if (gpio_is_valid(data->ctp_power_io.gpio)) {
...
*/
  ...
  data->irq_gpio.gpio = of_get_named_gpio(np, "gsensor_int1", 0);	
  if (!gpio_is_valid(data->irq_gpio.gpio))
		pr_err("%s: irq_gpio is invalid. \n",__func__ );
  else
    data->int_number = data->irq_gpio.gpio;
...
int input_set_power_enable(enum input_sensor_type *input_type, u32 enable)
{
	...
	pr_debug("[%s] input_type = %i; enable = %i (data = %p; ldo = %p; power_io = %i\n",__func__,*input_type,enable,data,ldo,power_io);
