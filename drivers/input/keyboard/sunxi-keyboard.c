...
//------add by guanjian for hall sensor and change by A-GAN--------
int  aw_send_hall_sensor_near_key(void)
{
	if (!sunxikbd_dev)
		return -1;
	input_report_key(sunxikbd_dev, KEY_COVER, 1);
	input_sync(sunxikbd_dev);
/*
	input_report_key(sunxikbd_dev, KEY_HALL_SENSOR_NEAR, 1);
	input_sync(sunxikbd_dev);
	printk("report a powerkey to system ------------------\n");
	input_report_key(sunxikbd_dev, KEY_HALL_SENSOR_NEAR, 0);
	input_sync(sunxikbd_dev);
*/
	return 0;
}

int  aw_send_hall_sensor_leave_key(void)
{
	if (!sunxikbd_dev)
		return -1;

	input_report_key(sunxikbd_dev, KEY_COVER, 0);
	input_sync(sunxikbd_dev);
/*
	input_report_key(sunxikbd_dev, KEY_HALL_SENSOR_LEAVE, 1);
	input_sync(sunxikbd_dev);
	printk("report a powerkey to system ------------------\n");
	input_report_key(sunxikbd_dev, KEY_HALL_SENSOR_LEAVE, 0);
	input_sync(sunxikbd_dev);
*/
	return 0;
}

EXPORT_SYMBOL(aw_send_hall_sensor_near_key);
EXPORT_SYMBOL(aw_send_hall_sensor_leave_key);
...
static int sunxi_keyboard_suspend(struct device *dev)
{
	...
	if ((check_scene_locked(SCENE_WLAN_STANDBY) == 0) || (check_scene_locked(SCENE_NORMAL_STANDBY) == 0)) {
			printk("normal standby, enable wakeup source !!\n");
			if(enable_wakeup_src(CPU0_KEY_SRC, 0) != 0){
				printk("enable CPU0_KEY_SRC wakeup src failed\n");
			}else{
				printk("enable CPU0_KEY_SRC wakeup src ok\n");
			}
		return 0;
	}
...
static int sunxi_keyboard_resume(struct device *dev)
{
	...
	if ((check_scene_locked(SCENE_WLAN_STANDBY) == 0) || (check_scene_locked(SCENE_NORMAL_STANDBY) == 0)) {
		disable_wakeup_src(CPU0_KEY_SRC, 0);
		printk("resume from normal standby!!\n");
...

{
	...
	set_bit(EV_KEY, sunxikbd_dev->evbit);
	set_bit(EV_REL, sunxikbd_dev->evbit);
	set_bit(KEY_POWER, sunxikbd_dev->keybit);

	/**add for obook hall-sensor 2017-01-12 check pmu gpio statute */
	set_bit(KEY_HALL_SENSOR_NEAR , sunxikbd_dev->keybit);	
	set_bit(KEY_HALL_SENSOR_LEAVE , sunxikbd_dev->keybit);
	/**/
	set_bit(KEY_COVER , sunxikbd_dev->keybit);
...
#else
#endif
	if (request_irq(key_data->irq_num, sunxi_isr_key, IRQF_NO_SUSPEND, "sunxikbd", NULL)) {
		err = -EBUSY;
