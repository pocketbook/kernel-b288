...
struct ctp_config_info icn85xx_config_info = {
	.input_type = CTP_TYPE,
	.name = NULL,
	.int_number = 0,
};
...
#define dprintk(level_mask,fmt,arg...)    if(unlikely(debug_mask & level_mask)) \
        printk("[elan_debug]"fmt, ## arg)
/* Addresses to scan */
#define ICN85XX_I2C_ADDR 0x48
 union{
	unsigned short dirty_addr_buf[2];
	const unsigned short normal_i2c[2];
}icn85xx_i2c_addr = {{ICN85XX_I2C_ADDR,I2C_CLIENT_END},};	
...
static void icn85xx_ts_suspend()
{
	...
	if ((check_scene_locked(SCENE_WLAN_STANDBY) == 0) || (check_scene_locked(SCENE_NORMAL_STANDBY) == 0)){
		...
		printk(KERN_ERR "[elan] enable ctp wakeup src\n", __func__);
		if(enable_wakeup_src(CPUS_GPIO_SRC, icn85xx_config_info.irq_gpio.gpio) != 0){
			printk(KERN_ERR "[elan] enable ctp wakeup src failed\n", __func__);
		}else{
    ...
    input_set_int_enable(&(icn85xx_config_info.input_type), 0);
    icn85xx_write_reg(ICN85xx_REG_PMODE, PMODE_HIBERNATE); 
    /*add by A-GAN  When super standby turn off */
		input_set_power_enable(&(icn85xx_config_info.input_type), 0);
		/*end add */
...
***********************************************************************************************/
static void icn85xx_ts_resume(void)
{
    struct icn85xx_ts_data *icn85xx_ts = i2c_get_clientdata(this_client);
    int i;
    int ret = -1;
	...
	//end
	/*add by A-GAN  When super standby turn off */
		input_set_power_enable(&(icn85xx_config_info.input_type), 1);
		/*end add */
   ...
   input_set_int_enable(&(icn85xx_config_info.input_type), 1);
...
static int icn85xx_request_irq(struct icn85xx_ts_data *icn85xx_ts)
{
    ...
    err = request_irq(icn85xx_ts->irq, icn85xx_ts_interrupt, IRQ_TYPE_EDGE_FALLING| IRQF_NO_SUSPEND, "icn85xx_ts", icn85xx_ts);
    if (err < 0) 
    {
...
static int icn85xx_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	int ret = 0;
	u32 val;
	struct icn8672_platdata_resource *pdata = client->dev.platform_data;
   ...
   icn85xx_ts->irq_gpio = icn85xx_config_info.irq_gpio.gpio;
   icn85xx_ts->rst_gpio = icn85xx_config_info.wakeup_gpio.gpio;
    if(gpio_is_valid(icn85xx_ts->irq_gpio))
   	{			
			ret = gpio_request(icn85xx_ts->irq_gpio, "icn85xx_ts->irq_gpio");
			if(ret < 0)
	...
	icn85xx_ts->revert_x_flag = revert_x_flag;
 	...
 	icn85xx_ts->xy_swap = exchange_x_y_flag;
	
    icn85xx_trace(" icn85xx: %dx%d  revert_x=%d,revert_y=%d,xy_swap=%d, firmware_type=%d---\n",icn85xx_ts->screen_max_x,icn85xx_ts->screen_max_y,icn85xx_ts->revert_x_flag,icn85xx_ts->revert_y_flag,icn85xx_ts->xy_swap,icn85xx_ts->firmware_type);
    ...
    init_proc_node();
#endif
    fb_register_client(&icn85xx_ts_fb_notifier);
...
static int icn85xx_ts_remove(struct i2c_client *client)
{
	...
	fb_unregister_client(&icn85xx_ts_fb_notifier);
...
static struct i2c_driver icn85xx_ts_driver = {
    ...
    .address_list   = icn85xx_i2c_addr.normal_i2c,
};
/**
 * ctp_wakeup - function
 *
...
{
	dprintk(DEBUG_INIT," %s:status:%d,ms = %d\n",__func__,status,ms);

	if (status == 0) {

		if(ms == 0) {
			__gpio_set_value(icn85xx_config_info.wakeup_gpio.gpio, 0);
		}else {
			__gpio_set_value(icn85xx_config_info.wakeup_gpio.gpio, 0);
			msleep(ms);
			__gpio_set_value(icn85xx_config_info.wakeup_gpio.gpio, 1);
		}
	}
	if (status == 1) {
		if(ms == 0) {
...
{
    ctp_print_info(icn85xx_config_info,DEBUG_INIT);

	fwname = icn85xx_config_info.name;
...
#endif
	twi_id = icn85xx_config_info.twi_id;
    screen_max_x = icn85xx_config_info.screen_max_x;
    screen_max_y = icn85xx_config_info.screen_max_y;
    revert_x_flag = icn85xx_config_info.revert_x_flag;
    revert_y_flag = icn85xx_config_info.revert_y_flag;
    exchange_x_y_flag = icn85xx_config_info.exchange_x_y_flag;
...
{ 
	...
	printk(KERN_ALERT "---icn85xx_ts_init--------\n");
	if (input_fetch_sysconfig_para(&(icn85xx_config_info.input_type))) {
		printk("%s: ctp_fetch_sysconfig_para err.\n", __func__);
		return 0;
	} else {
		ret = input_init_platform_resource(&(icn85xx_config_info.input_type));
		if (0 != ret) {
			printk("%s:ctp_ops.init_platform_resource err. \n", __func__);    
		}
	}
	if (icn85xx_config_info.ctp_used == 0) {
...
static void __exit icn85xx_ts_exit(void)
{
	...
	input_free_platform_resource(&(icn85xx_config_info.input_type));
	return;
}
