...
void cyttsp4_get_touch_record_(struct device *dev, int rec_no, int *rec_abs)
{
	...
	for (abs = CY_TCH_X; abs < CY_TCH_NUM_ABS; abs++) {
		...
		//hxm add
		 if(CY_TCH_Y==abs)
		{ 
			//printk("%s:XXX befor line=%d  x=%d y=%d \n",__func__,__LINE__,rec_abs[CY_TCH_X],rec_abs[CY_TCH_Y]);
			rec_abs[CY_TCH_Y]=1024-rec_abs[CY_TCH_Y];
			printk("%s:XXX after line=%d,, x=%d y=%d \n",__func__,__LINE__,rec_abs[CY_TCH_X],rec_abs[CY_TCH_Y]);
		}
		//printk("%s:XXX after line=%d x=%d y=%d \n",__func__,__LINE__,rec_abs[CY_TCH_X],rec_abs[CY_TCH_Y]);
...
static int release_exclusive(struct cyttsp4_core_data *cd, void *ownptr)
{
	...
	cytts4_printk(" exclusive_dev %p freed\n",
		 cd->exclusive_dev);
...
static int cyttsp4_wait_bl_heartbeat(struct cyttsp4_core_data *cd)
{
	...
	/* wait heartbeat */
	cytts4_printk(" wait heartbeat..cd->mode=%d.\n",cd->mode);
...
struct cyttsp4_sysinfo *cyttsp4_request_sysinfo_(struct device *dev)
{
	...
	mutex_unlock(&cd->system_lock);
	cytts4_printk("ready = %d \n",ready);
	if (ready)
		return &cd->sysinfo;
	cytts4_printk(" nullllll\n");
	return NULL;
}
...
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	int rc = cyttsp4_toggle_low_power_(cd, mode);
	if (rc < 0)
		...
		u8 *cmd_buf, size_t cmd_size)
{
	...
	if (cmd_param_size > cmd_data_len) {
		dev_err(dev, "%s: Parameter size overflow:%d max:%d\n",
		...
		size_t param_ret_size, int timeout_ms)
{
	...
	if (param_ret_size > cmd_data_len) {
		dev_err(dev, "%s: Return data size overflow:%d max:%d\n",
...
{
	u8 mode[2];
	int rc = cyttsp4_adap_read(cd, CY_REG_BASE, &mode, sizeof(mode));
	cytts4_printk(" rc=%d mode[0]=%d mode[1]=%d \n",rc,mode[0],mode[1]);
	if (rc) {
...
static int cyttsp4_startup_(struct cyttsp4_core_data *cd)
{
    ...
    cytts4_printk("  mode=%d\n",cd->mode);
reset:
	cytts4_printk(" retry =%d \n",retry);
	if (retry != CY_CORE_STARTUP_RETRY_COUNT)
		dev_dbg(cd->dev, "[cytts4===]  %s: Retry %d\n", __func__,
	...
	if (rc < 0) {
	    ...
	    cytts4_printk(" rc1=%d \n",rc1);
		if (rc1) {
			dev_err(cd->dev, "%s: Fail to read rc1=%d\n",
				__func__, rc1);
	...
	cytts4_printk("cyttsp4_get_sysinfo_regs_ rc=%d\n",rc);
...
exit_no_wd:
	if (!detected)
		rc = -ENODEV;
	cytts4_printk("  cyttsp4_exit startup rc=%d.. \n",rc);
...
static int cyttsp4_core_rt_suspend(struct device *dev)
{
	struct cyttsp4_core_data *cd = dev_get_drvdata(dev);
	int rc;
	//printk("%s:  line=%d  get_suspend_state=%d  \n",__func__,__LINE__,get_suspend_state());
	cytts4_printk();
...
static int cyttsp4_core_suspend(struct device *dev)
{
	...
	if (get_suspend_state() != PM_SUSPEND_IDLE){
		...
		/* if POWEROFF_ON_SLEEP set disable_irq before sleep,
		 * else disable_irq after sleep because irq required to sleep */
		if (!(cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP)) {
			if (cd->irq_enabled) {
				cd->irq_enabled = false;
				/*
...
static void cyttsp4_early_suspend(struct early_suspend *h)
{
    ...
    int i;
    if (get_suspend_state() != PM_SUSPEND_IDLE ){
		printk("%s:  line=%d  number_of_open_input_device=%d  \n",__func__,__LINE__,cd->number_of_open_input_device);
...
#if 1
   /* Initialize IRQ */
	cd->irq = 175;//gpio_to_irq(cd->cpdata->irq_gpio);
	if (cd->irq < 0)
		return -EINVAL;

	cd->irq_enabled = true;

	cytts4_printk(" initialize threaded irq=%d irq_gpio=%d\n", cd->irq,cd->cpdata->irq_gpio);
	//hxma add set_irq_mode
	...
	else
	{
		...
		irq_flags = (IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND) ;//IRQF_TRIGGER_FALLING|IRQF_SHARED;
		//irq_flags = IRQF_TRIGGER_FALLING | IRQF_ONESHOT;
	}
	//rc = devm_request_threaded_irq(dev,
	//				175, cyttsp4_hard_irq, cyttsp4_irq,
	//				(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), "xd4-core", cd);

	rc = request_threaded_irq(cd->irq, cyttsp4_hard_irq, cyttsp4_irq,
			irq_flags, dev_name(dev), cd);
	cytts4_printk(" request_threaded_irq rc=%d \n",rc);
	...
	config_info.dev = &(md->input->dev);// 
	cytts4_printk(",input_type = %d, dev=%x dev_name=%s",config_info.input_type,config_info.dev,dev_name(config_info.dev));

	//irq_flags = IRQF_TRIGGER_FALLING|IRQF_SHARED;
	//rc = request_threaded_irq(&(config_info.input_type), cyttsp4_hard_irq, cyttsp4_irq,
	//		irq_flags, dev_name(dev), cd);
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	if (!pdata || !pdata->core_pdata || !pdata->mt_pdata) {
		dev_err(dev, "[cytts4===] %s:line=%d Missing platform data\n", __func__,__LINE__);
		rc = -ENODEV;
		goto error_no_pdata;
	}
	...
	if ((cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP)
		...
		dev_err(dev, "[cytts4===] %s: No power function with POWEROFF_ON_SLEEP flag\n",
			__func__);
		rc = -EINVAL;
		goto error_power;
	}
	...
	if (cd->cpdata->init) {
		cytts4_printk("\n");
		dev_dbg(cd->dev, "[cytts4===] %s: line=%d Init HW\n", __func__,__LINE__);
		rc = cd->cpdata->init(cd->cpdata, 1, cd->dev);
		cytts4_printk("init rc =%d \n",rc);
	} else {
		dev_info(cd->dev, "[cytts4===] %s: line=%d No HW INIT function\n", __func__,__LINE__);
		rc = 0;
	}
	if (rc < 0)
		dev_err(cd->dev, "[cytts4===] %s: line=%d HW Init fail r=%d\n", __func__,__LINE__, rc);

	/* Call platform detect function */
	if (cd->cpdata->detect) {	
		cytts4_printk(" detect hw\n");

		dev_info(cd->dev, "%s: Detect HW\n", __func__);
		rc = cd->cpdata->detect(cd->cpdata, cd->dev,
				cyttsp4_platform_detect_read);
		cytts4_printk("detect rc =%d \n",rc);
		if (rc) {
			dev_info(cd->dev, "[cytts4===] %s: line=%d No HW detected\n", __func__,__LINE__);
			rc = -ENODEV;
			goto error_detect;
		}
	}
...
/////////////////////////////////////
	//cytts4_printk("start cyttsp4_setup_irq_gpio \n");
	rc = cyttsp4_setup_irq_gpio(cd);
	cytts4_printk("cyttsp4_setup_irq_gpio rc=%d\n",rc);
	if (rc < 0) {
	...
	rc = cyttsp4_startup(cd);
	cytts4_printk("cyttsp4_startup rc =%d mode =%d \n",rc,cd->mode);
	...
	if (rc == -ENODEV) {
		cytts4_printk("Fail initial startup \n");
		
		//dev_err("[cytts4===] %s: line=%d Fail initial startup r=%d\n",
		//	__func__, __LINE__,rc);
		goto error_startup;
	}
	cytts4_printk("\n");
	...
	if (rc < 0) {
		dev_err(dev, "[cytts4===] %s: line=%d Error, fail sysfs init\n", __func__,__LINE__);
		goto error_startup;
	}
	...
	if (rc < 0) {
		dev_err(dev, "[cytts4===] %s:  line=%d Error, fail btn probe\n", __func__,__LINE__);
		goto error_startup_mt;
	}

	rc = cyttsp4_proximity_probe(dev);
	if (rc < 0) {
		dev_err(dev, "[cytts4===] %s:  line=%d Error, fail proximity probe\n", __func__,__LINE__);
