	...
	struct cyttsp4_touch *touch)
{
	...
	printk("%s: line=%d flip=%s inv-x=%s inv-y=%s x=%04X(%d) y=%04X(%d)\n",
		__func__,__LINE__, flipped ? "true" : "false",
		...
		int value)
{
	...
	printk("%s %s line=%d  sig=%d value=%d\n",__FILE__,__FUNCTION__,__LINE__,sig,value);
...
static void cyttsp4_get_mt_touches(struct cyttsp4_mt_data *md, int num_cur_rec)
{
    ...
    printk("%s %s line=%d  num_cur_rec=%d\n",__FILE__,__FUNCTION__,__LINE__,num_cur_rec);
 ...
 int cyttsp4_mt_probe(struct device *dev)
{
	...
	//config_info.dev = &(md->input->dev);//             ->input->dev);
