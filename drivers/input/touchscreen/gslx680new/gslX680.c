...
struct gslX680_fw_array {
	...
	{"gslX680"  ,  ARRAY_SIZE(GSLX680E_B100_FW),GSLX680E_B100_FW},
};
...
unsigned int *gslX680_config_data[16] = {
    ...
    gsl_config_data_id_b100_1024600,
};
...
static void process_gslX680_data(struct gsl_ts *ts)
{
	...
	for(i= 0;i < (touches > MAX_FINGERS ? MAX_FINGERS : touches);i++)
	{
    ...
    #endif
		dprintk(DEBUG_OTHERS_INFO,"tp-gsl id=%u x:%u y:%u\n",id, x, y);
		if(1 <=id && id <= MAX_CONTACTS)
		{
			...
			record_point(x, y , id);
		#endif
			dprintk(DEBUG_OTHERS_INFO,"tp-gsl id=%u x_new:%u y_new:%u\n",id, x_new, y_new);
...
static void gsl_ts_xy_worker(struct work_struct *work)
{
	...
	if (!being_work && device_may_wakeup(&(ts->client->dev))){
		if (0 == int_cnt++) {
			pm_stay_awake(&(ts->client->dev));
			report_data(ts, 10, 10, 10, 1);//send a invalid data to wake up android
			input_sync(ts->input);
		}
		goto schedule;
	}
...
static int gsl_ts_suspend(struct device *dev)
{
	...
	/*if (check_scene_locked(SCENE_NORMAL_STANDBY) == 0||check_scene_locked(SCENE_WLAN_STANDBY) == 0){
		//enable ctp wake up source
		dprintk(DEBUG_SUSPEND,"%s,enable ctp wakeup src\n",__func__);
		if(enable_wakeup_src(CPUS_GPIO_SRC, config_info.irq_gpio.gpio)!=0){
			dprintk(DEBUG_SUSPEND,"%s,enable wakeup src failed!\n",__func__);
		}else{
