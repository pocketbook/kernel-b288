...
int mytotalsize=0, myusedsize=0;
uint8_t *myptr;
...
static int cyttsp5_device_access_probe(struct device *dev, void **data)
{
	...
	mytotalsize =
		8 + sizeof(*dad) +
		8 + sizeof(*configurations) + 
		8 + sizeof(*cmcp_info) + 
		8 + sizeof(*result) + 
		8 + sizeof(*test_case_field_array) * MAX_CASE_NUM +
		8 + sizeof(*test_case_search_array) * MAX_CASE_NUM +
		8 + tx_num * sizeof(struct gd_sensor) +
		8 + rx_num * sizeof(struct gd_sensor) +
		8 + (tx_num * rx_num + 1) * sizeof(int32_t) +
		8 + tx_num * sizeof(int32_t) +
		8 + tx_num * sizeof(int32_t) +
		8 + rx_num * sizeof(int32_t) +
		8 + rx_num * sizeof(int32_t) +
		8 + btn_num * sizeof(int32_t) +
		8 + btn_num * sizeof(int32_t) +
		8 + rx_num * tx_num * sizeof(int32_t) +
		8 + tx_num * rx_num  * sizeof(int32_t);

	myptr = kzalloc(mytotalsize, GFP_KERNEL);
	if (!myptr) {
		rc = -ENOMEM;
		goto cyttsp5_device_access_probe_data_failed;
	}

	dad = my_alloc(sizeof(*dad));
	configurations = my_alloc(sizeof(*configurations));
	dad->configs = configurations;
	cmcp_info = my_alloc(sizeof(*cmcp_info));
	...
	result = my_alloc(sizeof(*result));
	dad->result = result;

	test_case_field_array = my_alloc(sizeof(*test_case_field_array) * MAX_CASE_NUM);
	test_case_search_array = my_alloc(sizeof(*test_case_search_array) * MAX_CASE_NUM);

	cmcp_info->gd_sensor_col = (struct gd_sensor *) my_alloc(tx_num * sizeof(struct gd_sensor));
	cmcp_info->gd_sensor_row = (struct gd_sensor *) my_alloc(rx_num * sizeof(struct gd_sensor));
	cmcp_info->cm_data_panel = my_alloc((tx_num * rx_num + 1) * sizeof(int32_t));
	cmcp_info->cp_tx_data_panel = my_alloc(tx_num * sizeof(int32_t));
	cmcp_info->cp_tx_cal_data_panel = my_alloc(tx_num * sizeof(int32_t));
	cmcp_info->cp_rx_data_panel = my_alloc(rx_num * sizeof(int32_t));
	cmcp_info->cp_rx_cal_data_panel = my_alloc(rx_num * sizeof(int32_t));
	cmcp_info->cm_btn_data = my_alloc(btn_num * sizeof(int32_t));
	cmcp_info->cp_btn_data = my_alloc(btn_num * sizeof(int32_t));
	cmcp_info->cm_sensor_column_delta = my_alloc(rx_num * tx_num * sizeof(int32_t));
	cmcp_info->cm_sensor_row_delta = my_alloc(tx_num * rx_num  * sizeof(int32_t));
