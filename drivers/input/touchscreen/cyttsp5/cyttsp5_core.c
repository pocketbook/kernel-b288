...
#undef dev_info
#undef dev_err
#define dev_info(dev, s...) printk("CY:INFO " s)
#define dev_err(dev, s...) printk("CY:ERR " s)
#endif
...
static int _tp_enabled = 1;

extern uint32_t touch_irq_gpio;
...
struct work_struct cyttsp5_work;
		...
		u8 param_id, u32 param_value, u8 param_size)
{
	...
	list_for_each_entry(param, &cd->param_list, node) {
		if (param->id == param_id) {
			...
			dev_info(cd->dev, "%s: Update parameter id:%d value:%d size:%d\n",
	...
	dev_info(cd->dev, "%s: Add parameter id:%d value:%d size:%d\n",
		__func__, param_id, param_value, param_size);
		...
		struct cyttsp5_hid_output *hid_output)
{
	...
	//if (hid_output->reset_expected) udelay(1000);
...
static int cyttsp5_si_get_btn_data(struct cyttsp5_core_data *cd)
{
	...
	dev_info(cd->dev, "%s: get btn data\n", __func__);
...
static void cyttsp5_si_put_log_data(struct cyttsp5_core_data *cd)
{
	...
	dev_info(cd->dev, "%s: pip_ver_major =0x%02X (%d)\n", __func__,
		cydata->pip_ver_major, cydata->pip_ver_major);
	dev_info(cd->dev, "%s: pip_ver_minor =0x%02X (%d)\n", __func__,
		cydata->pip_ver_minor, cydata->pip_ver_minor);
	dev_info(cd->dev, "%s: fw_pid =0x%04X (%d)\n", __func__,
		cydata->fw_pid, cydata->fw_pid);
	dev_info(cd->dev, "%s: fw_ver_major =0x%02X (%d)\n", __func__,
		cydata->fw_ver_major, cydata->fw_ver_major);
	dev_info(cd->dev, "%s: fw_ver_minor =0x%02X (%d)\n", __func__,
		cydata->fw_ver_minor, cydata->fw_ver_minor);
	dev_info(cd->dev, "%s: revctrl =0x%08X (%d)\n", __func__,
		cydata->revctrl, cydata->revctrl);
	dev_info(cd->dev, "%s: fw_ver_conf =0x%04X (%d)\n", __func__,
		cydata->fw_ver_conf, cydata->fw_ver_conf);
	dev_info(cd->dev, "%s: bl_ver_major =0x%02X (%d)\n", __func__,
		cydata->bl_ver_major, cydata->bl_ver_major);
	dev_info(cd->dev, "%s: bl_ver_minor =0x%02X (%d)\n", __func__,
		cydata->bl_ver_minor, cydata->bl_ver_minor);
	dev_info(cd->dev, "%s: jtag_id_h =0x%04X (%d)\n", __func__,
		cydata->jtag_id_h, cydata->jtag_id_h);
	dev_info(cd->dev, "%s: jtag_id_l =0x%04X (%d)\n", __func__,
		cydata->jtag_id_l, cydata->jtag_id_l);
	for (i = 0; i < CY_NUM_MFGID; i++)
		dev_info(cd->dev, "%s: mfg_id[%d] =0x%02X (%d)\n", __func__, i,
			cydata->mfg_id[i], cydata->mfg_id[i]);
	dev_info(cd->dev, "%s: post_code =0x%04X (%d)\n", __func__,
		cydata->post_code, cydata->post_code);

	dev_info(cd->dev, "%s: electrodes_x =0x%02X (%d)\n", __func__,
		scd->electrodes_x, scd->electrodes_x);
	dev_info(cd->dev, "%s: electrodes_y =0x%02X (%d)\n", __func__,
		scd->electrodes_y, scd->electrodes_y);
	dev_info(cd->dev, "%s: len_x =0x%04X (%d)\n", __func__,
		scd->len_x, scd->len_x);
	dev_info(cd->dev, "%s: len_y =0x%04X (%d)\n", __func__,
		scd->len_y, scd->len_y);
	dev_info(cd->dev, "%s: res_x =0x%04X (%d)\n", __func__,
		scd->res_x, scd->res_x);
	dev_info(cd->dev, "%s: res_y =0x%04X (%d)\n", __func__,
		scd->res_y, scd->res_y);
	dev_info(cd->dev, "%s: max_z =0x%04X (%d)\n", __func__,
		scd->max_z, scd->max_z);
	dev_info(cd->dev, "%s: origin_x =0x%02X (%d)\n", __func__,
		scd->origin_x, scd->origin_x);
	dev_info(cd->dev, "%s: origin_y =0x%02X (%d)\n", __func__,
		scd->origin_y, scd->origin_y);
	dev_info(cd->dev, "%s: panel_id =0x%02X (%d)\n", __func__,
		scd->panel_id, scd->panel_id);
	dev_info(cd->dev, "%s: btn =0x%02X (%d)\n", __func__,
		scd->btn, scd->btn);
	dev_info(cd->dev, "%s: scan_mode =0x%02X (%d)\n", __func__,
		scd->scan_mode, scd->scan_mode);
	dev_info(cd->dev, "%s: max_num_of_tch_per_refresh_cycle =0x%02X (%d)\n",
		__func__, scd->max_tch, scd->max_tch);

	dev_info(cd->dev, "%s: xy_mode =%p\n", __func__,
		si->xy_mode);
	dev_info(cd->dev, "%s: xy_data =%p\n", __func__,
		si->xy_data);
}
...
error:
	dev_info(cd->dev, "%s: CONFIG_VER:%04X\n", __func__, config_ver);
	return rc;
}

static int cyttsp5_get_mfg_data_(struct cyttsp5_core_data *cd)
{
	static u8 xcmd[] = { 0x04, 0x00, 0x0a, 0x00, 0x2f, 0x00, 0x22, 0x00, 0x00, 0x44, 0x00, 0x01 };
	...
	rc = cyttsp5_adap_write_read_specific(cd, sizeof(xcmd), xcmd, answer);
	if (rc) {
		dev_err(cd->dev, "%s: cyttsp5_adap_write_read_specific error\n", __func__);
		goto exit;
	}

	strlcpy(cd->mfgdata, &answer[0x0e], sizeof(cd->mfgdata));
exit:
	cyttsp5_hid_output_resume_scanning_(cd);
error:
	dev_info(cd->dev, "%s: MFG_DATA: '%s'\n", __func__, cd->mfgdata);
	return rc;
}
		...
		struct cyttsp5_core_data *cd, u8 *panel_id)
{
	...
	if (rc == -EPROTO && cd->response_buf[5] == ERROR_COMMAND) {
		dev_info(cd->dev, "%s: Get Panel ID command not supported\n",
			__func__);
		...
		struct cyttsp5_hid_desc *desc)
{
	...
	//while (1) {
	//	cd->hid_cmd_state = 1;
	//	t = wait_event_timeout(cd->wait_q, (cd->hid_cmd_state == 0), msecs_to_jiffies(20));
	//	if (IS_TMO(t)) break;
	//}
	...
	dev_info(dev, "%s: HID [ %02x %02x %02x %02x %02x %02x %02x %02x ]\n",
		 __func__,
		cd->response_buf[0],
		cd->response_buf[1],
		cd->response_buf[2],
		...
		u8 *report_desc, size_t len)
{
	...
	dev_info(cd->dev, "%s: Report descriptor length: %zu\n",
		__func__, len);
...
static int setup_report_descriptor(struct cyttsp5_core_data *cd)
{
	...
	for (i = CY_TCH_X; i < CY_TCH_NUM_ABS; i++) {
		...
		if (field) {
			...
			dev_info(cd->dev, "%s: ofs:%zu size:%zu min:%zu max:%zu bofs:%zu report:%d",
	...
	dev_info(cd->dev, "Features: easywake:%d noise_metric:%d tracking_heatmap:%d sensor_data:%d\n",
...
static int cyttsp5_get_report_descriptor_(struct cyttsp5_core_data *cd)
{
	...
	dev_info(cd->dev, "%s: %d reports found in descriptor\n", __func__,
		cd->num_hid_reports);
	...
	for (t = 0; t < cd->num_hid_reports; t++) {
		...
		for (j = 0; j < report->num_fields; j++) {
			...
			dev_info(cd->dev, "  Collections Phys:%08X App:%08X Log:%08X\n",
				field->collection_usage_pages
...
static void cyttsp5_queue_startup_(struct cyttsp5_core_data *cd)
{
	if (cd->startup_state == STARTUP_NONE) {
		...
		dev_info(cd->dev, "%s: startup_state = %d\n", __func__,
			cd->startup_state);
	}
}
		...
		enum cyttsp5_atten_type type, int mode)
{
	...
	//dev_info(cd->dev, "%s: check list type=%d mode=%d\n",
		//__func__, type, mode);
	spin_lock(&cd->spinlock);
	list_for_each_entry_safe(atten, atten_n,
		...
		if (!mode || atten->mode & mode) {
			spin_unlock(&cd->spinlock);
			//dev_info(cd->dev, "%s: attention for '%s'", __func__,
				//dev_name(atten->dev));
...
static int cyttsp5_core_sleep_(struct cyttsp5_core_data *cd)
{
	...
	printk("[cyttsp5_core_sleep] POWEROFF_ON_SLEEP=%d\n", (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP)?1:0);
	...
	if (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		...
		regulator_disable( config_info.ctp_power_ldo);
	} else {
...
static int cyttsp5_core_sleep(struct cyttsp5_core_data *cd)
{
	...
	if (release_exclusive(cd, cd->dev) < 0)
		...
		dev_info(cd->dev, "%s: pass release exclusive\n", __func__);
...
exit:
	...
	dev_info(cd->dev, "%s: gesture_id = %d, gesture_data_length = %d\n",
...
static int parse_touch_input(struct cyttsp5_core_data *cd, int size)
{
	...
	//dev_info(cd->dev, "%s: Received touch report\n", __func__);
	if (!si->ready) {
		dev_err(cd->dev,
...
static int cyttsp5_parse_input(struct cyttsp5_core_data *cd)
{
	...
	/* check reset */
	if (size == 0) {
		dev_info(cd->dev, "%s: Reset complete\n", __func__);
...
static struct cyttsp5_core_data *__cd;

static void cyttsp5_ts_work_func(struct work_struct *work)
{
	int rc = 0;

	while (__cd->cpdata->irq_stat(__cd->cpdata, __cd->dev) == CY_IRQ_ASSERTED_VALUE) {
		rc = cyttsp5_read_input(__cd);
		if (rc == 0) cyttsp5_parse_input(__cd);
	}
...
#if 1		
	//printk("CY:IRQ ***\n");
	if (!cyttsp5_check_irq_asserted(__cd)) {
		printk("CY:IRQ not asserted\n");
		return IRQ_HANDLED;
	}
	input_set_int_enable(&(config_info.input_type), 0);
	last_irq = jiffies;

	queue_work(cyttsp5_wq, &cyttsp5_work);
...
static int cyttsp5_restore_parameters_(struct cyttsp5_core_data *cd)
{
	...
	list_for_each_entry(param, &cd->param_list, node) {
		spin_unlock(&cd->spinlock);
		dev_info(cd->dev, "%s: Parameter id:%d value:%d\n",
...
static int cyttsp5_core_wake_(struct cyttsp5_core_data *cd)
{
	...
	printk("[cyttsp5_core_wake] POWEROFF_ON_SLEEP=%d\n", (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP)?1:0);

	//printk("CY:CY_CORE_FLAG_POWEROFF_ON_SLEEP=%d\n", (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP)?1:0);
	if (cd->cpdata->flags & CY_CORE_FLAG_POWEROFF_ON_SLEEP) {
		regulator_enable(config_info.ctp_power_ldo);
		msleep(1);
...
exit:
	cyttsp5_hid_output_resume_scanning_(cd);
error:
	dev_info(cd->dev, "%s: CRC: ebid:%d, crc:0x%04X\n",
...
static int cyttsp5_core_suspend(struct device *dev)
{
	...
	printk("[cyttsp5_core_suspend] _tp_enabled=%d\n", _tp_enabled);
	if (! _tp_enabled) return 0;

	if ((jiffies - last_irq) < HZ/2) return -EBUSY;
	...
	if ((check_scene_locked(SCENE_WLAN_STANDBY) == 0) || (check_scene_locked(SCENE_NORMAL_STANDBY) == 0)){
		...
		printk(KERN_ERR "[cyttsp5_core_suspend] enable ctp wakeup src\n");
		if(enable_wakeup_src(CPUS_GPIO_SRC, config_info.irq_gpio.gpio) != 0){
			printk(KERN_ERR "[cyttsp5_core_suspend] enable ctp wakeup src failed\n");
		}else{
		} 
		touch_irq_gpio = config_info.irq_gpio.gpio;
		return 0;
	} 
	printk("[cyttsp5_core_suspend] poweroff\n");
	touch_irq_gpio = 0;
	cd->cpdata->flags |= CY_CORE_FLAG_POWEROFF_ON_SLEEP;
...
static int cyttsp5_core_resume(struct device *dev)
{
	...
	printk("[cyttsp5_core_resume] _tp_enabled=%d\n", _tp_enabled);
	if (! _tp_enabled) return 0;

        printk("hxm add cyttsp5_core_resume\n");
	if ((check_scene_locked(SCENE_WLAN_STANDBY) == 0) || (check_scene_locked(SCENE_NORMAL_STANDBY) == 0)){
		...
		unsigned long action, void *data)
{
	...
	if (action == PM_SUSPEND_PREPARE) {
		dev_info(cd->dev, "%s: Suspend prepare\n", __func__);
...
static ssize_t cyttsp5_mfg_data_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	struct cyttsp5_core_data *cd = dev_get_drvdata(dev);
	ssize_t ret;
	
	ret = cyttsp5_get_mfg_data_(cd);
	if (ret) return -EIO;
...
static struct device_attribute attributes[] = {
	...
	__ATTR(mfg_data, S_IRUGO, cyttsp5_mfg_data_show, NULL),
};
...
{
	struct device *dev = cd->dev;
	//struct ctp_config_info *configinfo=cd->cpdata->configinfo; 

	unsigned long irq_flags;
	int rc;

	/* Initialize IRQ */
	cd->irq = gpio_to_irq(config_info.int_number);
	if (cd->irq < 0)
		return -EINVAL;

	cd->irq_enabled = true;

	dev_info(dev, "%s: initialize threaded irq 1111=%d\n", __func__, cd->irq);
	...
	//rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq,(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND),
		//dev_name(dev), cd);
	if(cd!=NULL)
	dev_info(dev, "%s: initialize threaded irq 222=%d\n", __func__, cd->irq);
	else
	dev_info(dev, "%s: initialize threaded irq 222=%d\n", __func__, cd->irq);
	//rc = input_request_int(&(config_info.input_type), cyttsp5_irq,(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), cd->cpdata);
	rc = request_threaded_irq(cd->irq, NULL, cyttsp5_irq,	irq_flags, dev_name(dev), cd);
	printk(" request_threaded_irq rc=%d \n",rc);
	//rc = input_request_int(&(config_info.input_type), cyttsp4_irq,
			//(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), cd);
        //rc = devm_request_irq(cd->cpdata->configinfo.dev, cd->irq, cyttsp5_irq,(IRQF_TRIGGER_FALLING | IRQF_NO_SUSPEND), dev_name(dev), cd);
	
	//dev_info(dev, "%s: initialize threaded irq3333=%d\n", __func__, cd->irq);

	//rc = devm_request_irq(dev, cd->irq , cyttsp5_irq,
			      //irq_flags, "PA3_EINT", cd);
	
	if (rc < 0)
		dev_err(dev, "%s: Error, could not request irq\n", __func__);
       else{
                printk("success request irq");
	}
	return rc;
}
		...
		u16 irq, size_t xfer_buf_size)
{
	...
	cyttsp5_wq = create_singlethread_workqueue("cyttsp5_wq");
	INIT_WORK(&cyttsp5_work, cyttsp5_ts_work_func);
	...
	dev_info(dev, "%s: register touch power callbacks\n", __func__);
	register_touchpower_callback(tp_enable, is_tp_enabled, dev);
