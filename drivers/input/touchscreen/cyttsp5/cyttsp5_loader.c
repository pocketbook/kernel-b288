...
static const char *manual_upgrade_status = "idle";
...
static void _cyttsp5_firmware_cont(const struct firmware *fw, void *context)
{
	...
	if (header_size >= (fw->size + 1)) {
		...
		manual_upgrade_status = "error";
		goto cyttsp5_firmware_cont_release_exit;
	}

	manual_upgrade_status = "loading";
	ret = cyttsp5_upgrade_firmware(dev, &(fw->data[header_size + 1]),
		fw->size - (header_size + 1));

	manual_upgrade_status = ret ? "error" : "success";
...
static DEVICE_ATTR(manual_upgrade, S_IRUSR | S_IWUSR,
	cyttsp5_manual_upgrade_show, cyttsp5_manual_upgrade_store);
#endif
