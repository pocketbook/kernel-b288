...
#undef dev_warn
#undef dev_err
#define dev_dbg(dev, s...)
//#define dev_info(dev, s...) printk("MMC:I " s)
#define dev_info(dev, s...)
#define dev_warn(dev, s...) printk("MMC:W " s)
#define dev_err(dev, s...) printk("MMC:E " s)
...
static void pin_setconf(uint32_t port, uint32_t pin, uint32_t mode, uint32_t pull)
{
	volatile uint32_t *modereg = (volatile u32 *) (0xf1c20800 + port * 0x24 + (pin / 8) * 4);
	volatile uint32_t *pullreg = (volatile u32 *) (0xf1c20800 + port * 0x24 + 0x1c + (pin / 16) * 4);
	uint32_t modeshift = (pin & 7) * 4;
	uint32_t pullshift = (pin & 15) * 2;
	*modereg = (*modereg & ~(0xf << modeshift)) | (mode << modeshift);
	*pullreg = (*pullreg & ~(0x3 << pullshift)) | (pull << pullshift);
} 

static void sdc0_set_pins(uint32_t mode, uint32_t pull)
{
	printk("sdc0_set_pins(%d)\n", mode, pull);
	pin_setconf(SDC0_PORT, SDC0_D0, mode, pull);
	pin_setconf(SDC0_PORT, SDC0_D1, mode, pull);
	pin_setconf(SDC0_PORT, SDC0_D2, mode, pull);
	pin_setconf(SDC0_PORT, SDC0_D3, mode, pull);
	pin_setconf(SDC0_PORT, SDC0_CLK, mode, pull);
	pin_setconf(SDC0_PORT, SDC0_CMD, mode, pull);
}
...
static void sunxi_mmc_set_ios(struct mmc_host *mmc, struct mmc_ios *ios)
{
	...
	dev_info(mmc_dev(mmc), "sdc%d set ios: "
	...
	case MMC_POWER_ON:
		dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios(MMC_POWER_ON)\n");
		break;

	case MMC_POWER_UP:
		dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios(%d,MMC_POWER_UP)\n", host->phy_index);
		if (host->power_on) {
			dev_info(mmc_dev(mmc), "already powered on");
			break;
		}
		...
		if (host->phy_index == 0) {
			sdc0_set_pins(SDC0_ON, 1);
		} else if (!IS_ERR(host->pins_default)) {
	...
	case MMC_POWER_OFF:
		 dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios(%d,MMC_POWER_OFF)\n", host->phy_index);
		if (!host->power_on || host->dat3_imask) {
			dev_info(mmc_dev(mmc), "power_on=%d dat3_imask=%d\n", host->power_on, host->dat3_imask);
			break;
		}
		...
		if (host->phy_index == 0) {
			sdc0_set_pins(SDC0_OFF, 0);
		} else if (!IS_ERR(host->pins_sleep)) {
			dev_info(mmc_dev(mmc), "pinctrl_select_state\n");
		...
		if (!IS_ERR(mmc->supply.vqmmc)) {
			dev_info(mmc_dev(mmc), "disable vqmmc\n");
		...
		if (!IS_ERR(mmc->supply.vmmc)) {
			dev_info(mmc_dev(mmc), "disable vmmc\n");
			rval = mmc_regulator_set_ocr(mmc, mmc->supply.vmmc, 0);
			if (rval) {
				dev_err(mmc_dev(mmc),
					"Could not disable vmmc\n");
				return;
			}
		}
	...
	case MMC_BUS_WIDTH_1:
		dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios:MMC_BUS_WIDTH_1\n");
		mmc_writel(host, REG_WIDTH, SDXC_WIDTH1);
		break;
	case MMC_BUS_WIDTH_4:
		dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios:MMC_BUS_WIDTH_4\n");
		mmc_writel(host, REG_WIDTH, SDXC_WIDTH4);
		break;
	case MMC_BUS_WIDTH_8:
		dev_info(mmc_dev(mmc), "sunxi_mmc_set_ios:MMC_BUS_WIDTH_8\n");
		mmc_writel(host, REG_WIDTH, SDXC_WIDTH8);
		break;
	}

	dev_info(mmc_dev(host->mmc), "REG_WIDTH: 0x%08x \n",
		...
		if(strcmp(mmc_hostname(mmc),"mmc1")==0){
				...
				if (!ctx || !gpio_is_valid(ctx->cd_gpio))
					return 0;		
	                         ...
	                         if(enable_wakeup_src(CPUS_GPIO_SRC,ctx->cd_gpio) != 0){
					printk(KERN_ERR "[sunxi_mmc]   %s wakeup src failed\n",mmc_hostname(mmc));
				}else{
					printk(KERN_ERR "[sunxi_mmc]   %s wakeup src success\n",mmc_hostname(mmc));
				} 	
			      return 0;
			}
                }
