...
{
	sbus_printk(XRADIO_DBG_NIY, "set blocksize=%u\n", size);
...
#else
	ret = xradio_request_irq(&(self->func->dev), self);
	if (!ret) {
		/* Hack to access Fuction-0 */
		u8 cccr;
		int func_num = self->func->num;
		sbus_printk(XRADIO_DBG_NIY, "%s:xradio_request_irq success.\n",
					__func__);
		...
		if (ret) {
			xradio_free_irq(&(self->func->dev), self);
		...
		sbus_printk(XRADIO_DBG_ERROR, "%s:xradio_request_irq failed(%d).\n",
				__func__, ret);
	}
#endif
	sdio_release_host(self->func);
			      ...
			      struct sbus_priv **sdio_priv)
{
	...
	if (sdio_self.load_state == SDIO_UNLOAD) {
		...
		if (wait_event_interruptible_timeout(sdio_self.init_wq,
			sdio_self.load_state == SDIO_LOAD, 4*HZ) <= 0) {
			sbus_printk(XRADIO_DBG_WARN, "sdio probe second!\n");
			xradio_sdio_detect(1);
			if (wait_event_interruptible_timeout(sdio_self.init_wq,
				sdio_self.load_state == SDIO_LOAD, 4*HZ) <= 0) {
...
void sbus_sdio_deinit(void)
{
	sbus_printk(XRADIO_DBG_TRC, "%s\n", __func__);
	if (sdio_self.load_state != SDIO_UNLOAD) {
		struct mmc_card *card = sdio_self.func->card;
		...
		sdio_self.load_state = SDIO_UNLOAD;
		xradio_wlan_power(0);
		mmc_manual_remove_card(host, card);
	} else {
