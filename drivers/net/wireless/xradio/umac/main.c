...
static char *xradio_iface_param = "wlan%d";
module_param_named(iface, xradio_iface_param, charp, S_IRUGO);
...
int mac80211_register_hw(struct ieee80211_hw *hw)
{
	...
	strncpy(ifname, xradio_iface_param, sizeof(ifname));
	printk("[%s] iface = %s\n",__func__,ifname);

	/* add one default STA interface if supported */
	if (local->hw.wiphy->interface_modes & BIT(NL80211_IFTYPE_STATION)) {
		result = mac80211_if_add(local, ifname/*"wlan%d"*/, NULL,
