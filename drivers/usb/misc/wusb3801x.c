...
#define WUSB3801_I2C_NAME "sunxi-wusb3801"
...
#define WUSB3801_SLAVE_ADDR0            0xc0>>1
#define WUSB3801_SLAVE_ADDR1            0xd0>>1
...
#define POWER_SUPPLY_PROP_INPUT_CURRENT_MAX  1  //hxm add for test
static int wusb3801_power_set_icurrent_max(struct wusb3801_chip *chip,
						int icurrent)
{
/*
...
static irqreturn_t wusb3801_interrupt(int irq, void *data)
{
	 struct wusb3801_chip *chip = (struct wusb3801_chip *)data;
        printk(" wusb3801_interrupt irq 0000=%d\n",irq);	
	if (!chip) {
...
static int wusb3801_parse_dt(struct wusb3801_chip *chip)
{
	...
	u32 val = 0;
	int rc = 0;
	dev_err(cdev, "np->name=%s.\n",np->name);


	
	data->int_gpio = of_get_named_gpio_flags(np,"irq-gpio", 0,(enum of_gpio_flags *)&config);	
	...
	rc = of_property_read_u32(np,"init-mode", &val);
	...
	rc = of_property_read_u32(np,"host-current", &val);
	...
	rc = of_property_read_u32(np,"drp-toggle-time", &val);
	data->dttime = (u8)val;
			...
			const struct i2c_device_id *id)
{
	...
	dev_info(cdev,"hxm chip->pdata->int_gpio=%d \n",chip->pdata->int_gpio);
	dev_info(cdev,"hxm chip->irq_gpio=%d \n", chip->irq_gpio);
	if (chip->irq_gpio < 0) {
...
static int wusb3801_i2c_detect(struct i2c_client *client, struct i2c_board_info *info)
{
	...
	if(1 == adapter->nr)
	{
		printk("%s: Detected chip %s at adapter %d, address 0x%02x\n",
			 __func__, WUSB3801_I2C_NAME, i2c_adapter_id(adapter), client->addr);

		strlcpy(info->type, WUSB3801_I2C_NAME, I2C_NAME_SIZE);
		return 0;
	}else{
		return -ENODEV;
	}
