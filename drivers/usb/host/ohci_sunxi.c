...
static struct workqueue_struct *ohci_resume_wq = NULL;
static struct delayed_work ohci_resume_work;

void ohci_resume_work_function(struct work_struct *work);
...
static int sunxi_ohci_hcd_probe(struct platform_device *pdev)
{
	...
	ohci_resume_wq = create_singlethread_workqueue("ohci-resume-wq");
	if (ohci_resume_wq == NULL) {
		DMSG_PANIC("ERR: %s, ohci_resume_wq is null\n", __func__);
		return -1;
	}
	INIT_DELAYED_WORK(&ohci_resume_work, ohci_resume_work_function);
...
static int sunxi_ohci_hcd_remove(struct platform_device *pdev)
{
	...
	if(ohci_resume_wq) {
		flush_workqueue(ohci_resume_wq);
		destroy_workqueue(ohci_resume_wq);
		ohci_resume_wq = NULL;
	}
...
struct sunxi_hci_hcd *r_sunxi_ohci = NULL;
struct usb_hcd *r_hcd = NULL;

void ohci_resume_work_function(struct work_struct *work)
{
#ifdef  SUNXI_USB_FPGA
	fpga_config_use_hci(r_sunxi_ohci);
#endif
	sunxi_start_ohci(r_sunxi_ohci);

	set_bit(HCD_FLAG_HW_ACCESSIBLE, &r_hcd->flags);
	ohci_resume(r_hcd, false);
}
...
static int sunxi_ohci_hcd_resume(struct device *dev)
{
	...
	r_hcd = dev_get_drvdata(dev);
	if(r_hcd == NULL){
		DMSG_PANIC("ERR: hcd is null\n");
		return 0;
	}

	r_sunxi_ohci = dev->platform_data;
	if(r_sunxi_ohci == NULL){
		DMSG_PANIC("ERR: sunxi_ohci is null\n");
		return 0;
	}

	if(r_sunxi_ohci->probe == 0){
		DMSG_INFO("[%s]: is disable, can not resume\n",
			r_sunxi_ohci->hci_name);
		return 0;
	}

	if(r_sunxi_ohci->wakeup_suspend){
		DMSG_INFO("[%s]: controller not suspend, need not resume\n", r_sunxi_ohci->hci_name);

		if(r_sunxi_ohci->clk_usbohci12m && r_sunxi_ohci->clk_hoscx2){
			clk_set_parent(r_sunxi_ohci->clk_usbohci12m, r_sunxi_ohci->clk_hoscx2);
		}

		scene_unlock(&ohci_standby_lock[r_sunxi_ohci->usbc_no]);
		disable_wakeup_src(CPUS_USBMOUSE_SRC, 0);

	}else{
		DMSG_INFO("[%s]: sunxi_ohci_hcd_resume\n", r_sunxi_ohci->hci_name);

		queue_delayed_work(ohci_resume_wq, &ohci_resume_work, HZ/10);
