...
static struct axp_regulator_info axp22_regulator_info[] = {
	AXP22_DCDC(1,  1600, 3400, 100,  DCDC1, 0, 5,  DCDC1EN, 0x02, 0x02,
			0x00,     0,    0,   0,   0x80,  0x1, 0x37, 0, 0, 0),
...
static int axp22_regulator_probe(struct platform_device *pdev)
{
	...
	//set 3.0V to dcdc1
	ret = axp_regmap_read(axp_dev->regmap, AXP22_DC1OUT_VOL,&val);
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	val = 0x0E;
	ret = axp_regmap_write(axp_dev->regmap, AXP22_DC1OUT_VOL,val);
	printk("%s %s line=%d ret=%d \n",__FILE__,__FUNCTION__,__LINE__,ret);

//E091 OPEN DLDO2 tf card
	#if 0
        //ʹ��
	ret = axp_regmap_read(axp_dev->regmap, AXP22_LDO_DC_EN2,&val);
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	val |= 0x10;	
	ret = axp_regmap_write(axp_dev->regmap, AXP22_LDO_DC_EN2,val); 
	printk("%s %s line=%d ret=%d \n",__FILE__,__FUNCTION__,__LINE__,ret);  
	ret = axp_regmap_read(axp_dev->regmap, AXP22_LDO_DC_EN2,&val); 
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
        //���õ�ѹ
	ret = axp_regmap_read(axp_dev->regmap, AXP22_DLDO2OUT_VOL,&val);
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	val = 0x1a;	
	ret = axp_regmap_write(axp_dev->regmap, AXP22_DLDO2OUT_VOL,val); 
	printk("%s %s line=%d ret=%d \n",__FILE__,__FUNCTION__,__LINE__,ret);  
	ret = axp_regmap_read(axp_dev->regmap, AXP22_DLDO2OUT_VOL,&val); 
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	#endif
//e091 open epd power PMU DLDO3 
        //ʹ��
	ret = axp_regmap_read(axp_dev->regmap, AXP22_LDO_DC_EN2,&val);
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	val |= 0x20;	
	...
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
        ...
        //���õ�ѹ
	ret = axp_regmap_read(axp_dev->regmap, AXP22_DLDO3OUT_VOL,&val);
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
	val = 0x1a;	
	ret = axp_regmap_write(axp_dev->regmap, AXP22_DLDO3OUT_VOL,val); 
	printk("%s %s line=%d ret=%d \n",__FILE__,__FUNCTION__,__LINE__,ret);  
	ret = axp_regmap_read(axp_dev->regmap, AXP22_DLDO3OUT_VOL,&val); 
	printk("%s %s line=%d ret=%d val=0x%x\n",__FILE__,__FUNCTION__,__LINE__,ret,val);
