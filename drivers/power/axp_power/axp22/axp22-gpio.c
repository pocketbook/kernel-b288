...
//--------add by guanjuan for hall_sensor--------
int axp22_read_gpio0_status(void)
{
	struct axp_dev *cur_axp_dev = get_pmu_cur_dev(axp22_pmu_num);
	struct axp_regmap *map = cur_axp_dev->regmap;
	u8 status;
	axp_regmap_read(map,AXP_GPIO01_STATE,&status);
	status &= 0x1;
	return status;
}
//--------end
...
static int axp22_gpio_probe(struct platform_device *pdev)
{
	...
	for (i = 0; i < ARRAY_SIZE(axp22_gpio_irq); i++) {
		...
		if (ret != 0) {
			DBG("failed to request %s IRQ %d: %d\n", axp22_gpio_irq[i].name, irq, ret);
			goto out_irq;
		}

		DBG("Requested %s IRQ %d: %d\n", axp22_gpio_irq[i].name, irq, ret);
	}
	...
	if (IS_ERR_OR_NULL(axp22_gpio_irqchip)) {
		DBG("axp22_gpio_irqchip: not enough memory\n");
