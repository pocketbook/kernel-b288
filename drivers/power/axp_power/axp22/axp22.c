...
/*
 * returns:
 * 0 - no power supply or battery full (device should go to poweroff)
 * 1 - power key was pressed (device shpuld go to reboot)
 */
static int axp_charging_waiter(void) {
		...
		if ((axp_regmap_read(axp22_pm_power->regmap, AXP22_STATUS, &val) != 0) || !(val & (AXP22_STATUS_ACVA | AXP22_STATUS_USBVA))) {
			printk("[axp] No external power sources!\n");

		} else {
			printk("[axp] full charge first and then send power-off command!\n");
			disable_irq(axp22_pm_power->irq);

			//clean PEK interrupt status
			ret = axp_regmap_set_bits(axp22_pm_power->regmap, AXP22_INTSTS3, (1 << 0) | (1 << 1));

			int tics = 0;
			const char *p = "..........";

			while (true) {
				printk("[axp] Charging %.*s                 \r",tics++ % 10,p);

				//check external power status
				ret = axp_regmap_read(axp22_pm_power->regmap, AXP22_STATUS, &val);
				if (ret < 0) {
					printk("\n[axp] i2c read error %i!\n", ret);
					break;
				}

				if(!(val & (AXP22_STATUS_ACEN | AXP22_STATUS_USBEN))) {
					printk("\n[axp] External source was disabled!\n");
					break;
				}

				//check charging process
				ret = axp_regmap_read(axp22_pm_power->regmap, AXP22_MODE_CHGSTATUS, &val);
				if (ret < 0) {
					printk("\n[axp] i2c read error %i!\n", ret);
					break;
				}
				...
				if ((val & (1 << 7))) {
					printk("\n[axp] AXP chip is over-temperature! Will be power off\n");
					break;
				}

				//check power key
				ret = axp_regmap_read(axp22_pm_power->regmap, AXP22_INTSTS3, &val);
				if (ret < 0) {
					printk("\n[axp] i2c read error %i!\n", ret);
					break;
				}
...
{
	uint8_t chip_id, dcdc2_ctl, voff_set;
	int err;
	...
	printk("AXP22: set cutoff voltage to 2.9V\n");
	err = axp_regmap_read(axp22->regmap, AXP22_VOFF_SET, &voff_set);
	if (err) {
		pr_err("read voff_set register failed!\n");
	} else {
		voff_set &= ~7;
		voff_set |= 3;
		axp_regmap_write(axp22->regmap, AXP22_VOFF_SET, voff_set);
	}
...
static s32 axp22_usb_det(void)
{
	...
	//printk("[%s] AXP22_STATUS = %hhx; %hhi;\n",__func__,value,(value & 0x20));
