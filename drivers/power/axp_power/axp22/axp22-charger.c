...
static int axp22_charger_init(struct axp_dev *axp_dev)
{
	...
	if (0 != axp22_config.pmu_bat_temp_enable) {
		val &= (~(1 << 2)); // set TS Pin function

		//set TS Pin Current Output while charging
		val &= (~(1 << 1));
		val |= (1 << 0);
	}
	...
	if (0 != axp22_config.pmu_bat_temp_enable) {
		printk("[%s] set pmu_bat_charge_ltf = %x; pmu_bat_charge_htf = %x\n",__func__,axp22_config.pmu_bat_charge_ltf,axp22_config.pmu_bat_charge_htf);
		axp_regmap_write(map, AXP22_VLTF_CHARGE,
				axp22_config.pmu_bat_charge_ltf * 10 / 128);
		axp_regmap_write(map, AXP22_VHTF_CHARGE,
				axp22_config.pmu_bat_charge_htf * 10 / 128);
//		axp_regmap_write(map, AXP22_VLTF_WORK,
//				axp259_config.pmu_bat_shutdown_ltf * 10 / 128);
//		axp_regmap_write(map, AXP22_VHTF_WORK,
