...
#ifdef CONFIG_SND_SUNXI_SOC_INTERNAL_SUN8IW10_AUDIOCODEC
extern void reset_audio_codec(void);
DECLARE_WORK(reset_audio_codec_w, (void(*)(struct work_struct *))reset_audio_codec);
#endif
...
static void axp_usb_ac_check_status(struct axp_charger_dev *chg_dev)
{
  ...
  printk("~~~~~ CH=%d USBPC=%d ADAP=%d USBDET=%d ACDET=%d INSHORT=%d\n",
      chg_dev->ac_charging, chg_dev->usb_pc_charging, chg_dev->usb_adapter_charging, chg_dev->usb_det, chg_dev->ac_det, chg_dev->in_short);

#ifdef CONFIG_SND_SUNXI_SOC_INTERNAL_SUN8IW10_AUDIOCODEC

  /*
   * USB switch chip, that routes usb data wires to codec.
   * It puts HPCOM to ground when charging cable is plugged in.
   * The audio codec falls to unworked state. So it should be reinited, when cable is detached.
   */
  static int prev_status = 0;
  int status = (chg_dev->usb_det | chg_dev->ac_det);
  if (status != prev_status) {
	  if (status) {
		  ...
		  //reinit audio codec
		  //reset_audio_codec();
		  schedule_work(&reset_audio_codec_w);
	  }
					...
					struct axp_supply_info *info)
{
	...
	if (chg_dev->bat_det) {
		schedule_delayed_work(&chg_dev->work, 0);
		schedule_delayed_work(&(chg_dev->usbwork), msecs_to_jiffies(5 * 1000));
	}
