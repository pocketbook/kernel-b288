...
static ssize_t key_write_store(struct class *class, struct class_attribute *attr,
		const char *buf, size_t count)
{
	u32 key_data[256];
	int i = 0, n = count;
	...
	while (n >= 8) {
		int ret = sscanf(buf, "%8x", &key_data[i]);
	...
	printk("sunxi_efuse_writen(%s,%d)\n", key_name, i * 4);
	int res = sunxi_efuse_writen(key_name, key_data, i * 4);
...
static struct class_attribute info_class_attrs[] = {
	...
	__ATTR(key_write, 0600, NULL, key_write_store),
	__ATTR_NULL,
};
