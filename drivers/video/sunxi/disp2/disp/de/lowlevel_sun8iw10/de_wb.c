...
{
	XLOG1("WB_eink_Set_Reg_Base: sel=%d, base=0x%x\n", sel, base + WB_OFFSET);
...
int WB_eink_Set_Para(unsigned int sel, __einkwb_config_t *cfg)
{
	...
	XLOG1("WB_eink_Set_Para: in_w=%d in_h=%d crop_w=%d crop_h=%d out_addr[%x,%x,%x]\n",
		in_w, in_h, crop_w, crop_h, out_addr[0], out_addr[1], out_addr[2]);
