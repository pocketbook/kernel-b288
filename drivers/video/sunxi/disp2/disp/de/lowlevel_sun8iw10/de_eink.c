...
int eink_decoder_start(phys_addr_t new_idx_addr, phys_addr_t wav_data_addr, struct eink_init_param *para)
{
        EINK_WUINT32(((para->ee.img_height-1) << 16) | (para->ee.img_width-1), ee_base + EE_IMG_SIZE);
        EINK_WUINT32(para->ee.wav_pitch, ee_base + EE_DEC_WAV_PITCH);
        EINK_WUINT32(new_idx_addr,ee_base + EE_DEC_IDX_ADDR);
        EINK_WUINT32(wav_data_addr + para->ee.wav_offset, ee_base + EE_DEC_WAV_ADDR);
        EINK_WUINT32(0x1,ee_base + EE_DEC_CTL);
	return para->ee.wav_offset;
}
...
int eink_edma_cfg(phys_addr_t wav_addr, struct eink_init_param *para)
{
        EINK_WUINT32(wav_addr, ee_base + EDMA_WAV_ADDR);
        EINK_WUINT32(para->ee.wav_pitch, ee_base + EDMA_WAV_PITCH);
        EINK_WUINT32(((para->ee.edma_wav_height-1) << 16) + (para->ee.edma_wav_width-1), ee_base + EDMA_WAV_SIZE);
        EINK_WUINT32((para->ee.edma_img_y << 16) + para->ee.edma_img_x ,ee_base + EDMA_IMG_COOR);
        EINK_WUINT32(((para->ee.edma_img_h-1) << 16) + (para->ee.edma_img_w-1) ,ee_base + EDMA_IMG_SIZE);
	return 0;
}
