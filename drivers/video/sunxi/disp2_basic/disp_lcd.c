...
struct disp_device* disp_get_lcd(u32 disp)
{
	...
	if (disp >= num_screens || !bsp_disp_feat_is_supported_output_types(disp, DISP_OUTPUT_TYPE_LCD)) {
		__inf("disp %d not support lcd output\n", disp);
		return NULL;
	}
...
static s32 lcd_clk_init(struct disp_device* lcd)
{
	...
	__inf("lcd %d clk init\n", lcd->disp);

	lcdp->clk_parent = x_clk_get_parent(lcdp->clk);
...
static s32 lcd_clk_config(struct disp_device* lcd)
{
	...
	if (lcdp->clk_parent) {
		x_clk_set_rate(lcdp->clk_parent, pll_rate);
		pll_rate_set = x_clk_get_rate(lcdp->clk_parent);
	}
	...
	x_clk_set_rate(lcdp->clk, lcd_rate_set);
	lcd_rate_set = x_clk_get_rate(lcdp->clk);
	if (LCD_IF_DSI == lcdp->panel_info.lcd_if) {
		dsi_rate_set = pll_rate_set / clk_info.dsi_div;
		dsi_rate_set = (0 == clk_info.dsi_rate)? dsi_rate_set:clk_info.dsi_rate;
		x_clk_set_rate(lcdp->dsi_clk0, dsi_rate_set);
	...
	DBG("target: pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld)\n", pll_rate, lcd_rate, dclk_rate, dsi_rate);
	DBG("real:   pll(%ld),clk(%ld),dclk(%ld) dsi_rate(%ld)\n", pll_rate_set, lcd_rate_set, dclk_rate_set, dsi_rate_set);
	return 0;
}
...
static s32 lcd_clk_enable(struct disp_device* lcd)
{
	...
	if (LCD_IF_LVDS == lcdp->panel_info.lcd_if) {
		...
		if (0 != ret) {
			__wrn("fail enable lvds's clock!\n");
			goto exit;
		}
		...
		if (lcdp->dsi_clk0) {
			...
			if (0 != ret) {
				__wrn("fail enable dsi's clock0!\n");
				goto exit;
			}
		} else {
			__wrn("dsi's clock0 is NULL!\n");
			goto exit;
		}
		if (lcdp->dsi_clk1) {
			ret = clk_prepare_enable(lcdp->dsi_clk1);
			if (0 != ret) {
				__wrn("fail enable dsi's clock1!\n");
				goto exit;
			}
		}
...
#if 0
extern int epdc_tcon_vb_interrupt(void);
extern int epdc_tcon_line_interrupt(void);
...
#endif
{
	...
	if (disp_al_lcd_query_irq(hwdev_index, LCD_IRQ_TCON0_VBLK, &lcdp->panel_info)) {
		epdc_tcon_vb_interrupt();
	} else if (disp_al_lcd_query_irq(hwdev_index, LCD_IRQ_TCON0_LINE, &lcdp->panel_info)) {
		epdc_tcon_line_interrupt();
	} else {
...
static s32 disp_lcd_enable(struct disp_device *lcd)
{
	...
	if (lcdp->lcd_panel_fun.cfg_panel_info)
		...
		__wrn("lcd_panel_fun[%d].cfg_panel_info is NULL\n", lcd->disp);
...
static s32 disp_lcd_set_panel_funs(struct disp_device* lcd, char *name, disp_lcd_panel_fun * lcd_cfg)
{
	...
	__inf("lcd %d, driver_name %s,  panel_name %s\n", lcd->disp, drv_name, name);
	...
	if (disp_lcd_is_used(lcd)) {
		...
		if (lcdp->panel_info.lcd_pwm_used) {
			...
			if (lcdp->panel_info.lcd_pwm_freq != 0) {
				...
				__wrn("lcd%d.lcd_pwm_freq is ZERO\n", lcd->disp);
...
s32 disp_init_lcd(disp_bsp_init_para * para)
{
	...
	if (NULL == lcds) {
		__wrn("malloc memory(%d bytes) fail!\n",
...
#endif
		__inf("lcd %d, irq_no=%d, irq_no_dsi=%d\n", disp, lcdp->irq_no, lcdp->irq_no_dsi);
...
void sunxi_tcon_enable(void)
{
	struct disp_device *plcd = disp_device_find(0, DISP_OUTPUT_TYPE_LCD);
	if (plcd) plcd->enable(plcd);
}
EXPORT_SYMBOL(sunxi_tcon_enable);

void sunxi_tcon_disable(void)
{
	struct disp_device *plcd = disp_device_find(0, DISP_OUTPUT_TYPE_LCD);
	if (plcd) schedule_work(&plcd->close_eink_panel_work);
}
EXPORT_SYMBOL(sunxi_tcon_disable);
