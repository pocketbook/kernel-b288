...
{
	XLOG1("de_dcsc_set_reg_base: sel=%d, base=0x%p\n", sel, base);
	if (is_in_smbl[sel])
...
int de_dcsc_apply(unsigned int sel, struct disp_csc_config *config)
{
	...
	XLOG1("de_dcsc_apply: sel=%d, in_fmt=%d, mode=%d, out_fmt=%d, mode=%d, range=%d\n",
...
int de_dcsc_init(disp_bsp_init_para *para)
{
	...
	for (screen_id=0; screen_id<device_num; screen_id++)
	{
		...
		{
			XLOG1("de_dcsc_init:  smbl=N id=%d, Dcsc_base=0x%p\n", screen_id, (void*)base);
