...
{
	XLOG1("de_ccsc_set_reg_base: sel=%d, chno=%d, base=0x%p\n", sel, chno, base);
...
{
	XLOG1("de_icsc_set_reg_base: sel=%d, chno=%d, base=0x%p\n", sel, chno, base);
...
int de_ccsc_apply(unsigned int sel, unsigned int ch_id, struct disp_csc_config *config)
{
	...
	XLOG1("de_ccsc_apply %d,%d [in_fmt=%d in_mode=%d out_fmt=%d out_mode=%d]\n", sel, ch_id, config->in_fmt, config->in_mode, config->out_fmt, config->out_mode);
	...
	for (screen_id=0; screen_id<device_num; screen_id++)
		for (ch_id=0; ch_id<vi_num[screen_id]; ch_id++)
		{
			...
			if (vep_support[screen_id][ch_id])
			{
				...
				XLOG1("de_ccsc_init: sel %d, Icsc_base[%d]=0x%p\n", screen_id, ch_id, (void*)base);
