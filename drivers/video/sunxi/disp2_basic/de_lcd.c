...
{
	s32 start_delay;
	XLOG1("tcon0_cfg_mode_auto sel=%d\n", sel);
	...
	XLOG1("  if=%d x=%d y=%d ht=%d hbp=%d vt=%d vbp=%d hspw=%d vspw=%d delay=%d\n",
		panel->lcd_hv_if,panel->lcd_x,panel->lcd_y,panel->lcd_ht,panel->lcd_hbp, panel->lcd_vt,panel->lcd_vbp,panel->lcd_hspw,panel->lcd_vspw,start_delay);
  if (panel->lcd_hv_if == LCD_HV_IF_CCIR656_2CYC)
  {
	...
	XLOG1("tcon0_cfg_mode_auto: start_delay=%d\n", start_delay);
...
static s32 tcon0_cfg_mode_tri(u32 sel, disp_panel_para * panel)
{
	...
	XLOG1("tcon0_cfg_mode_tri sel=%d\n", sel);
	...
	start_delay = (panel->lcd_vt -	panel->lcd_y -10-1) * panel->lcd_ht * 149 / tmp / 8;
...
s32 tcon0_cfg(u32 sel, disp_panel_para * panel)
{
	...
	lcd_dev[sel]->tcon_volume_ctl.bits.safe_period_fifo_num = tmp*15;
...
{
	XLOG1("tcon0_cpu_set_auto_mode sel=%d\n", sel);
	/* trigger mode 0 */
