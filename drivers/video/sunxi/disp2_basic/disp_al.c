...
{
	DDBG("disp_al_set_rtmx_base %d %x\n", disp, base);
...
{
	DDBG("disp_al_rtmx_set_addr %d %x\n", disp, addr0);
...
{
	DDBG("disp_al_set_eink_wb_base %d %x\n", disp, base);
...
{
	DDBG("disp_al_set_eink_wb_param %d (%d,%d) addr=%x\n", disp, w, h, addr);
...
{
	DDBG("disp_al_set_eink_base %d %x\n", disp, base);
...
int disp_al_eink_config(u32 disp, struct epdc_init_param* param)
{
	eink_config(1 /* 4-bit wf */, param->bus_width);
	return 0;
}
...
s32 disp_al_eink_start_calculate_index(
	struct area_info *area, int isfull,
	...
	struct eink_image_slot *last_image, struct eink_image_slot *current_image)
{
	...
	int flash_mode = isfull ? FLASH_GLOBAL : FLASH_LOCAL;

	tcurrent_img.addr = current_image->paddr;
	tcurrent_img.w = current_image->width;
	tcurrent_img.h = current_image->height;
	tcurrent_img.pitch = current_image->scanline;

	tlast_img.addr = last_image->paddr;
	tlast_img.w = last_image->width;
	tlast_img.h = last_image->height;
	tlast_img.pitch = last_image->scanline;

	memcpy((void*)&update_area, (void*)area, sizeof(struct area_info));

	DDBG("eink_start_idx: AREA=(%d.%d %d.%d) L=(%x:%dx%d) C=(%x:%dx%d) OIDX=%x NIDX=%x flash=%d\n",
		update_area.x_top, update_area.y_top, update_area.x_bottom, update_area.y_bottom,
		tlast_img.addr, tlast_img.w, tlast_img.h,
		tcurrent_img.addr, tcurrent_img.w, tcurrent_img.h,
		old_index_data_paddr, new_index_data_paddr,
		flash_mode);

	eink_start_idx(&tlast_img, &tcurrent_img, flash_mode, 0 /*win_en*/,
...
{
	//DDBG("disp_al_eink_pipe_enable %d\n", pipe_no);
	return eink_pipe_enable(pipe_no);
}

int disp_al_eink_pipe_disable(u32 disp, unsigned int pipe_no)
{
	//DDBG("disp_al_eink_pipe_disable %d\n", pipe_no);
	return eink_pipe_disable(pipe_no);
}

int disp_al_eink_pipe_config(u32 disp,  unsigned int pipe_no, struct area_info *area)
{
	//DDBG("disp_al_eink_pipe_config %d (%d,%d,%d,%d)\n", pipe_no, area->x_top, area->y_top, area->x_bottom, area->y_bottom);
	//__debug("set wave addr to pipeline, pipe_no=%d, wavefile_addr=0x%x\n", pipe_no, wav_file_addr);
	return eink_pipe_config(area, pipe_no);

}

int disp_al_eink_pipe_config_wavefile(u32 disp, phys_addr_t wav_file_addr, unsigned int pipe_no)
{
	//DDBG("disp_al_eink_pipe_config_wavefile addr=%x pipe=%d\n", wav_file_addr, pipe_no);
	return eink_pipe_config_wavefile(wav_file_addr, pipe_no);
}

int disp_al_eink_start_decode(unsigned int disp, phys_addr_t new_idx_addr,
								phys_addr_t wav_data_addr,
								struct epdc_init_param *param)
{
	//DDBG("disp_al_eink_start_decode newidx=%x wavdata=%x\n", new_idx_addr, wav_data_addr);
	return eink_decoder_start(new_idx_addr, wav_data_addr, param);
}

int disp_al_edma_init(unsigned int disp,struct epdc_init_param* param)
{
	//DDBG("disp_al_edma_init %d\n", param->bus_width);
	return eink_edma_init(param->bus_width);
}

int disp_al_eink_edma_cfg_addr(unsigned int disp, phys_addr_t wav_addr)
{
	//DDBG("disp_al_eink_edma_cfg_addr %d wav_addr=%x\n", disp, wav_addr);
	return  eink_edma_cfg_addr(wav_addr);
}


int disp_al_edma_config(unsigned int disp, phys_addr_t wave_data_addr, struct epdc_init_param *param)
{
...
bool disp_al_is_waveform_supported(int wf)
{
	return is_waveform_supported(wf);
}

int disp_al_get_waveform_data(unsigned int disp, int wf, int updmode, unsigned int temp,
				unsigned int *total_frames, phys_addr_t *wf_buf)
{
	return get_waveform_data(wf, updmode, temp, total_frames, wf_buf);
}

int disp_al_init_eink_ctrl_data_8(void *wavedata_buf, struct eink_timing_param *eink_timing_info)
{
	return init_eink_ctrl_data_8(wavedata_buf, eink_timing_info);
}

int disp_al_init_eink_ctrl_data_16(void *wavedata_buf, struct eink_timing_param *eink_timing_info)
{
...
int disp_al_lcd_cfg(u32 screen_id, disp_panel_para * panel, panel_extend_para *extend_panel)
{
	...
	//al_priv.output_fps[screen_id] = panel->lcd_dclk_freq * 1000 / panel->lcd_ht / panel->lcd_vt;
