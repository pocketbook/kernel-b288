...
#define SID_DBG(fmt, arg...) pr_err("%s()%d - "fmt, __func__, __LINE__, ##arg)
...
#define SUNXI_SIDC_BASE			0xf1c14000
#define SUNXI_SIDC_PRCTL (void __iomem *)(SUNXI_SIDC_BASE + 0x40)
#define SUNXI_SIDC_PRKEY (void __iomem *)(SUNXI_SIDC_BASE + 0x50)
#define SUNXI_SIDC_RDKEY (void __iomem *)(SUNXI_SIDC_BASE + 0x60)
#define SIDC_OP_LOCK 0xAC

uint32_t sid_efuse_key_wr(struct efuse_key_map *key_map, void *buf, u32 n)
{
	u32 reg_val;
	u32 i;
	u32 *indata = (u32 *) buf;

	SID_ERR("Write the key %s, len: %d [%08x %08x]\n", key_map->name, n, indata[0], indata[1]);
	for (i = 0; i < n; i += 4) {

		writel(indata[i], SUNXI_SIDC_PRKEY);

		udelay(10);

		reg_val = readl(SUNXI_SIDC_PRCTL);
		reg_val &= ~(((0x1ff) << 16) | 0x3);
		reg_val |= ((key_map->offset + i) << 16);
		writel(reg_val, SUNXI_SIDC_PRCTL);
		...
		while (readl(SUNXI_SIDC_PRCTL) & 0x1)
			;
...
/* read efuse to buf, return true if key is empty (if buf is NULL, only check for empty key) */
bool sid_efuse_key_rd(struct efuse_key_map *key_map, void *buf, u32 n)
{
	u32 i;
	u32 value = 0;
	u32 key_size = 0;
	...
	for (i = 0; i < key_size/4; i++) {
		...
		if (remainder <= 4) {
			if (dst) memcpy(&dst[i], &value, remainder);
			break;
		}
		if (dst) dst[i] = value;
		remainder -= 4;
...
/* n - the size of read_buf, in bytes */
s32 sunxi_efuse_writen(void *key_name, void *buf, u32 n)
{
	struct efuse_key_map *key_map = key_maps;

	if ((key_name == NULL) || (*(s8 *)key_name == 0)
			|| (n == 0) || (buf == NULL)) {
		SID_ERR("Invalid parameter. name: %p, write_buf: %p, size: %d\n",
				key_name, buf, n);
		return -EINVAL;
	}
	WARN_ON(n < 4);
	...
	if (key_map->size < n) {
		SID_ERR("No space in %s to write %d bytes.\n", (s8 *)key_name, n);
		return -1;
	}

	if (n == 0 || (n & 3) != 0) {
		SID_ERR("Wrong key length %d.\n", n);
		return -1;
	}

	if (! sid_efuse_key_rd(key_map, NULL, n)) {
		SID_ERR("Key is locked\n");
		return -EACCES;
	}
	sid_efuse_key_wr(key_map, buf, n);
	return 0;
}
EXPORT_SYMBOL(sunxi_efuse_writen);
