...
#define CONTRUL_POWER_UP_PING (0)
...
static int papyrus_resume_flag = 0;
...
static int papyrus_hw_read_temperature(struct pmic_sess *pmsess, int *t)
{
	...
	while(!papyrus_resume_flag && nwait) {
		msleep(5);
		nwait = nwait - 1;
		printk(KERN_ERR "wait papyrus_pm_resume papyrus_resume_flag = %d nwait = %d\n", papyrus_resume_flag, nwait);
	}
	...
	if (stat) {
		*t = -1;  //通信失败返回-1温度
		printk(KERN_ERR "papyrus_hw_read_temperature fail *t = %d stat = %d \n", *t, stat);
		return stat;
	}
...
{
	printk("65185----------------------driver start by guanj-----------------------0722\n");
	...
	pdata = kzalloc(sizeof(struct tps65185_platform_data), GFP_KERNEL);
	if (NULL == pdata) {
		pr_err("%s: fail to alloc pdata\n", __func__);
		stat = -ENOMEM;
		goto free_sess;
	}
	
	do_gettimeofday(&sess->standby_tv);
	printk("65185----------------------driver start by guanj-----------------------\n");
	...
	pdata->wake_up_pin = of_get_named_gpio_flags(np, "tps65185_wakeup", 0, (enum of_gpio_flags *)(&(pdata->wake_up_pin)));
	if (!gpio_is_valid(pdata->wake_up_pin))
		pr_err("%s: ctp_power_io is invalid. \n",__func__ );
	
	pdata->vcom_ctl_pin = of_get_named_gpio_flags(np, "tps65185_vcom", 0, (enum of_gpio_flags *)(&(pdata->vcom_ctl_pin)));
	if (!gpio_is_valid(pdata->vcom_ctl_pin))
		pr_err("%s: ctp_power_io is invalid. \n",__func__ );
	
#if (CONTRUL_POWER_UP_PING)		
	pdata->power_up_pin = of_get_named_gpio_flags(np, "tps65185_powerup", 0, (enum of_gpio_flags *)(&(pdata->power_up_pin)));
	printk("%s power_up_pin = %d\n",__func__,pdata->power_up_pin);
	if (!gpio_is_valid(pdata->power_up_pin))
		pr_err("%s: ctp_power_io is invalid. \n",__func__ );
#endif

	printk("%s: get wake_up_pin=%d, vcom_ctrl_pin=%d, power_up_pin=%d,\n", 
			__func__, pdata->wake_up_pin, pdata->vcom_ctl_pin, pdata->power_up_pin);
	sess->wake_up_pin = pdata->wake_up_pin;
	sess->vcom_ctl_pin = pdata->vcom_ctl_pin;
	sess->power_up_pin = pdata->power_up_pin;
	...
	printk("65185----------------------driver end by guanj-----------------------0722\n");
	return 0;
...
static int tps65185_get_sys_param()
{
