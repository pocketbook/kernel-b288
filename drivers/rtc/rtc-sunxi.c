...
static void __iomem *sunxi_rtc_base = 0;
...
int sunxi_rtc_get_user(int n, unsigned long *value)
{
	if (! sunxi_rtc_base) return -ENODEV;
	*value = readl(sunxi_rtc_base + SUNXI_RTC_USER(n));
	return 0;
}
EXPORT_SYMBOL_GPL(sunxi_rtc_get_user);

int sunxi_rtc_set_user(int n, unsigned long value)
{
	if (! sunxi_rtc_base) return -ENODEV;
	writel(value, sunxi_rtc_base + SUNXI_RTC_USER(n));
	return 0;
}
EXPORT_SYMBOL_GPL(sunxi_rtc_set_user);
...
static int sunxi_rtc_probe(struct platform_device *pdev)
{
	...
	sunxi_rtc_base = chip->base = devm_ioremap_resource(&pdev->dev, res);
	...
	tmp_data = REG_LOSCCTRL_MAGIC | EXT_LOSC_GSM | RTC_SOURCE_EXTERNAL;
...
//hxm add
unsigned long suspend_time_rtc=0,resume_time_rtc=0;
static int sunxi_rtc_suspend(struct platform_device *pdev,pm_message_t state)
{
	struct sunxi_rtc_dev *chip = platform_get_drvdata(pdev);
	u32 date, time;
	...
	printk("suspend Read hardware RTC time %04d-%02d-%02d %02d:%02d:%02d\n",
			rtc_tm->tm_year + 1900, rtc_tm->tm_mon + 1, rtc_tm->tm_mday,
			rtc_tm->tm_hour, rtc_tm->tm_min, rtc_tm->tm_sec);

	 err=rtc_valid_tm(rtc_tm);
         printk("rtc_valid_tm=%d\n",err);
	rtc_tm_to_time(rtc_tm, &suspend_time_rtc);
        printk("sunxi_rtc_suspend time=%lu\n",suspend_time_rtc);
	return err;
}
static int sunxi_rtc_resume(struct platform_device *pdev)
{
	struct sunxi_rtc_dev *chip = platform_get_drvdata(pdev);
	u32 date, time;
	struct rtc_time *rtc_tm;
	...
	printk("resume Read hardware RTC time %04d-%02d-%02d %02d:%02d:%02d\n",
			rtc_tm->tm_year + 1900, rtc_tm->tm_mon + 1, rtc_tm->tm_mday,
			rtc_tm->tm_hour, rtc_tm->tm_min, rtc_tm->tm_sec);

	 err=rtc_valid_tm(rtc_tm);
         printk("rtc_valid_tm=%d\n",err);
	rtc_tm_to_time(rtc_tm, &resume_time_rtc);
        printk("sunxi_rtc_resume time=%lu\n",resume_time_rtc);
        printk(" RTC sleep_time=%lu\n",resume_time_rtc-suspend_time_rtc);		 
